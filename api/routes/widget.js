...
<div style="text-align:center;color:#94a3b8"><p style="font-size:48px;margin:0">🤖</p><p>Widget not found.</p></div>
    ...
    const config = {
      ...
      suggestedPrompts: Array.isArray(chatbot.suggested_prompts) ? chatbot.suggested_prompts : [],
      faqs: Array.isArray(chatbot.faqs) ? chatbot.faqs : [],
    };

    res.setHeader('Content-Type', 'text/html; charset=utf-8');
    res.setHeader('X-Frame-Options', 'ALLOWALL');
    res.setHeader('Content-Security-Policy', "frame-ancestors *; default-src 'self' 'unsafe-inline' https:;");
