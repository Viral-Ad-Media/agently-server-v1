...
function getWidgetAvatarHtml(avatarLabel, fallbackInitial) {
  ...
  if (value.startsWith(CHATBOT_AVATAR_UPLOAD_PREFIX)) {
    ...
    if (isSafeDataImageSrc(dataUri)) {
      return `<span class="av-media"><img src="${escapeHtml(dataUri)}" alt="" loading="eager" decoding="async" onerror="this.style.display=\'none\';this.nextElementSibling.style.display=\'flex\'"/><span class="av-fallback">${initial}</span></span>`;
    }
  }
  if (isSafeDataImageSrc(value)) {
    return `<span class="av-media"><img src="${escapeHtml(value)}" alt="" loading="eager" decoding="async" onerror="this.style.display=\'none\';this.nextElementSibling.style.display=\'flex\'"/><span class="av-fallback">${initial}</span></span>`;
  }
  if (value.startsWith(CHATBOT_AVATAR_PREFIX)) {
    ...
    if (fileName)
      return `<span class="av-media"><img src="/chatbot-avatars/${escapeHtml(fileName)}" alt="" loading="eager" decoding="async" onerror="this.style.display=\'none\';this.nextElementSibling.style.display=\'flex\'"/><span class="av-fallback">${initial}</span></span>`;
  }
  return initial;
}
...
#cw{position:absolute;inset:0;width:100%;height:100%;max-width:none;max-height:none;background:#fff;border-radius:20px;box-shadow:0 12px 48px rgba(0,0,0,.18),0 2px 8px rgba(0,0,0,.08);display:flex;flex-direction:column;overflow:hidden;z-index:2147483647;transform-origin:bottom ${cfg.position};transition:transform .25s cubic-bezier(.34,1.56,.64,1),opacity .2s;}
...
.av img{width:100%;height:100%;object-fit:cover;display:block;} .av-media{width:100%;height:100%;display:block;position:relative}.av-fallback{display:none;width:100%;height:100%;align-items:center;justify-content:center;font-weight:800;font-size:14px;}
