...
// ─────────────────────────────────────────────────────────────
router.post('/outbound-twiml', asyncHandler(async (req, res) => {
...
router.get('/outbound-twiml', asyncHandler(async (req, res) => {
  res.setHeader('Content-Type', 'text/xml');
    ...
    try {
      ...
      if (agent) {
        ...
        if (!existing) {
          ...
          await db.rpc('increment_usage', { org_id: agent.organization_id, calls_inc: 1, minutes_inc: mins })
            .catch(() => {});
        }
      }
    } catch (e) {
...
router.post('/sms-inbound', asyncHandler(async (req, res) => {
  ...
  try {
    const db = getSupabase();
    const agent = await lookupAgentByPhone(isWhatsApp ? To?.replace('whatsapp:', '') : To);
    ...
    if (isWhatsApp && agent && Body) {
      ...
      const [faqRes] = await Promise.allSettled([
        db.from('faqs').select('question,answer').eq('voice_agent_id', agent.id).limit(30),
      ]);
...
router.get('/numbers/countries', requireAuth, asyncHandler(async (_req, res) => {
  const countries = await listSupportedCountries();
  res.json({ countries });
}));

// Search available numbers
router.get('/numbers/search', requireAuth, asyncHandler(async (req, res) => {
  const { country = 'US', type = 'Local', areaCode, contains, limit } = req.query;
  const numbers = await searchAvailableNumbers({
    country: country.toUpperCase(),
    type: type.charAt(0).toUpperCase() + type.slice(1).toLowerCase() === 'tollfree' ? 'TollFree' : type,
    areaCode,
    contains,
...
router.get('/numbers/owned', requireAuth, asyncHandler(async (_req, res) => {
  const numbers = await listOwnedNumbers();
  res.json({ numbers });
}));

// Purchase a number and assign it to a voice agent
router.post('/numbers/purchase', requireAuth, requireAdmin, asyncHandler(async (req, res) => {
  const { phoneNumber, voiceAgentId } = req.body;
  if (!phoneNumber) {
    return res.status(400).json({ error: { message: 'phoneNumber is required.' } });
  }

  const db = getSupabase();

  // Resolve agent
  const targetAgentId = voiceAgentId || req.organization.active_voice_agent_id;
  const { data: agent } = await db
    .from('voice_agents')
    .select('*')
    .eq('id', targetAgentId)
...
router.post('/numbers/assign', requireAuth, requireAdmin, asyncHandler(async (req, res) => {
  const { phoneSid, phoneNumber, voiceAgentId } = req.body;
  if (!phoneSid || !phoneNumber) {
    return res.status(400).json({ error: { message: 'phoneSid and phoneNumber are required.' } });
  }
  ...
  await db.from('voice_agents').update({
    ...
    updated_at: new Date().toISOString(),
  }).eq('id', targetAgentId).eq('organization_id', req.orgId);

  res.json({ success: true });
}));

// Release a number
router.delete('/numbers/:sid', requireAuth, requireAdmin, asyncHandler(async (req, res) => {
  const { sid } = req.params;

  const db = getSupabase();

  // Clear from voice agents first
  await db.from('voice_agents')
    .update({ twilio_phone_number: '', twilio_phone_sid: '', updated_at: new Date().toISOString() })
...
router.get('/calls', requireAuth, asyncHandler(async (req, res) => {
  ...
  const numbers = (agents || []).map(a => a.twilio_phone_number).filter(Boolean);
...
// ─────────────────────────────────────────────────────────────
router.get('/billing', requireAuth, asyncHandler(async (_req, res) => {
...
// ─────────────────────────────────────────────────────────────
router.post('/outbound', requireAuth, requireAdmin, asyncHandler(async (req, res) => {
  const { toPhone, customerName, voiceAgentId } = req.body;
  if (!toPhone) return res.status(400).json({ error: { message: 'toPhone is required.' } });
  ...
  if (!agent?.twilio_phone_number) {
    return res.status(400).json({ error: { message: 'This agent has no Twilio number assigned. Purchase one first.' } });
  }
...
// ─────────────────────────────────────────────────────────────
router.get('/voice-test', requireAuth, asyncHandler(async (req, res) => {
  const db = getSupabase();
