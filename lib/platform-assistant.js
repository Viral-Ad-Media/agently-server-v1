 ...
 */
/** Screenshots a reporter may attach. Hard limit, enforced again by a CHECK
 *  constraint on the column — this arrives from a public widget, so the client
 *  is not trusted to bound it. */
const MAX_SUPPORT_ATTACHMENTS = 2;
...
/** Private bucket. No read policy for tenants — the super-admin screen reads
 *  it with the service role and issues short-lived signed URLs. */
...
function sanitizeAttachments(input) {
  ...
  return input
    ...
    .filter((a) => ALLOWED_ATTACHMENT_MIME.has(String(a.mime || "")))
    .filter((a) => Number(a.bytes) > 0 && Number(a.bytes) <= MAX_ATTACHMENT_BYTES)
    .filter((a) => String(a.path || "").trim().length > 0)
    .slice(0, MAX_SUPPORT_ATTACHMENTS)
    .map((a) => ({
      path: String(a.path).trim().slice(0, 500),
      mime: String(a.mime),
