...
async function hasCardOnFile(db, organizationId) {
  ...
  try {
    ...
    // A blip and a misconfiguration both land here, and they need different
    // reactions. "Not configured", or a live/test key mismatch, is not going to
    // clear on its own: it means this gate has been open the whole time it has
    // been broken, and it will stay open until somebody fixes the key. Say so
    // at error level so it is not lost among transient warnings.
    const misconfigured =
      error?.code === "STRIPE_NOT_CONFIGURED" ||
      /test mode|live mode|No such customer|Invalid API Key|api_key/i.test(message);
    if (misconfigured) {
      console.error(
        "[activation-gate] CARD GATE IS OPEN — Stripe is misconfigured, so " +
          "card-on-file cannot be verified and every tenant is being treated " +
          "as having a card. Fix STRIPE_SECRET_KEY (it must be the live key " +
          "matching the live customer ids in billing_stripe_customers):",
        message,
      );
    } else {
      console.warn("[activation-gate] card lookup failed:", message);
    }
    // Still fail OPEN. Blocking a paying tenant because Stripe blipped is worse
    // than briefly allowing an ungated purchase, and switching to fail-closed
    // is a business decision about who gets locked out, not a code cleanup.
    return true;
  }
}
