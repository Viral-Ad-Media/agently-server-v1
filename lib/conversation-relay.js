...
async function rebuildSystemMessage({
  db,
  orgId,
  agentRow,
  leadRow,
  ...
  return {
    systemPrompt:
      ...
      "You are an AI receptionist. Be concise and helpful.",
    context,
  };
}
    ...
    if (type === "prompt") {
      ...
      try {
        ...
        if (messages.length === 0 || messages[0].role !== "system")
          messages.unshift({ role: "system", content: rebuilt.systemPrompt });
        else messages[0] = { role: "system", content: rebuilt.systemPrompt };

        promptCount += 1;
        const promptExternalId = `conversation-relay:${callSid || orgId}:${promptCount}`;
        let fullReply = "";
        await generateStreamingResponse(
          ...
          },
          {
            ...
            voiceAgentId: agentRow?.id,
            service: "conversation_relay",
            eventType: "openai_conversation_relay_tokens",
            route: "conversation_relay.prompt",
            externalId: promptExternalId,
            metadata: {
              call_sid: callSid || null,
              prompt_index: promptCount,
              runtime_path: "conversation_relay",
            },
          },
        );
        ...
        const replyCharacters = String(fullReply || "").length;
        if (replyCharacters > 0) {
          insertUsageEvent({
            organizationId: orgId,
            provider: "elevenlabs",
            service: "conversation_relay_tts",
            eventType: "elevenlabs_conversation_relay_observed_characters",
            source: "conversation_relay",
            externalId: `${promptExternalId}:tts-observed`,
            voiceAgentId: agentRow?.id || null,
            unit: "characters",
            quantity: replyCharacters,
            billable: false,
            metadata: {
              call_sid: callSid || null,
              prompt_index: promptCount,
              voice_provider:
                agentRow?.voice_provider || "twilio_conversation_relay",
              elevenlabs_voice_id:
                agentRow?.elevenlabs_voice_id || agentRow?.voice_id || null,
              note: "Observed assistant reply characters sent to Twilio ConversationRelay TTS. Non-billable until exact Twilio/ElevenLabs provider cost is reconciled.",
            },
          }).catch((err) => {
            console.warn(
              "[usage-ledger] ConversationRelay TTS character log skipped",
        ...
        const fallback =
          "I'm sorry, I'm having some trouble right now. Please leave a message and someone will follow up.";
        if (ws.readyState === 1) {
          ws.send(
  ...
  try {
    ...
    try {
      summary = await generateCallSummary(transcriptStr, "completed", {
        ...
        voiceAgentId: agentRow?.id,
        metadata: {
          route: "conversation_relay.saveCallRecord",
          call_sid: callSid || null,
        },
      });
    } catch {}
    ...
    if (leadId) {
      ...
      // "Booking Enquiry" is a real lead touch and belongs here; it must NOT
      // reach appointment_set, which asserts a booking nothing has made.
      if (
        ["Lead Captured", "Appointment Booked", "Booking Enquiry"].includes(outcome)
      ) {
        leadUpdates.status = "contacted";
        leadUpdates.crm_stage =
          outcome === "Appointment Booked" ? "appointment_set" : "contacted";
        leadUpdates.last_contacted_at = new Date().toISOString();
        leadUpdates.last_activity_at = new Date().toISOString();
        leadUpdates.ai_summary = summary;
      }
      await db
        .from("leads")
        .update(leadUpdates)
    ...
    const { data: savedCall } = await db
      ...
      .insert({
        ...
        metadata: {
          ...
          unanswered: containsUnansweredResponse(transcript),
        },
      })
      .select("id")
      .maybeSingle();
    ...
    if (leadId) {
      try {
        ...
        console.warn("[conversation-relay] CRM activity skipped:", e.message);
      }
    }

    await insertUsageEvent({
      organizationId: orgId,
      // We run on AWS now, not Railway. New runtime is logged as "aws" so the
      // Railway-era rows stay identifiable as what they were — those tenants
      // were genuinely billed for Railway while we were paying Railway.
      provider: "aws",
      service: "conversation_relay_runtime",
      eventType: "conversation_relay_runtime_seconds",
      source: "conversation_relay.save_call_record",
      externalId: `${callSid || orgId}:conversation-relay-runtime`,
      voiceAgentId: agentRow?.id || null,
      unit: "seconds",
      quantity: Math.max(0, Number(duration || 0)),
      // Not directly billable: the Lightsail bundle is a flat monthly price, so
      // there is no per-second rate to charge. These seconds are the driver
      // lib/usage-billing-engine.js apportions that flat bill by.
      billable: false,
      metadata: {
        call_sid: callSid || null,
        outcome,
        transcript_messages: Array.isArray(transcript) ? transcript.length : 0,
        runtime_path: "conversation_relay",
        note: "Application runtime seconds for Twilio ConversationRelay websocket session. Cost allocation is applied separately from exact provider ledger rows.",
      },
    }).catch((err) => {
      console.warn(
        "[usage-ledger] ConversationRelay runtime log skipped",
        err.message || String(err),
      );
    });
      ...
      await db
        ...
        .insert({
          ...
          question: transcript
            ...
            .slice(0, 2000),
          bot_response:
            transcript.filter((entry) => entry.speaker === "Agent").slice(-1)[0]
              ?.text || summary,
        })
        .catch(() => {});
    }
...
function containsUnansweredResponse(transcript) {
  return transcript.some(
    (entry) => entry.speaker === "Agent" && isUnanswered(entry.text),
  );
}
 ...
 *
 * N008: this used to return "Appointment Booked" when the transcript merely
 * CONTAINED the substring "appointment" or "book". No calendar was consulted,
 * because none exists. "I don't want to book an appointment" and "we can book
 * you in later" both produced the same label as a real booking — and that
 * label is not cosmetic, it writes crm_stage = appointment_set onto the lead
 * and drives the dashboard. The product did not book appointments but it
 * reported them.
 *
 * The rule now: an outcome that asserts something HAPPENED must come from a
 * fact, not from the transcript. `signals` carries those facts — a booking
 * reference, an actual transfer, a recorded voicemail. Text is used only for
 * the categories that are genuinely about what was SAID.
 *
 * Until appointment booking exists (a5), no caller can pass
 * signals.appointmentId, so "Appointment Booked" is simply never returned.
 * That is correct: the honest label for a caller who asked about booking is
 * that they enquired.
 *
 * @param {Array} transcript
 * @param {{appointmentId?: string, transferred?: boolean, voicemailRecorded?: boolean}} [signals]
 */
function determineOutcome(transcript, signals = {}) {
  const text = transcript
    .map((message) => message.text)
    .join(" ")
    .toLowerCase();

  // Facts first. Each of these is something the system OBSERVED, not something
  // it read in a sentence.
  if (signals.appointmentId) return "Appointment Booked";
  if (signals.transferred) return "Escalated";
  if (signals.voicemailRecorded) return "Voicemail";
  ...
  // Deliberately weaker than the old labels: the caller RAISED booking, which
  // is a real and useful signal, and it does not claim a booking was made.
...
function extractCapturedData(transcript) {
  ...
  const emailMatch = callerText.match(
