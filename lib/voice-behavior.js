...
function cleanOrganizationNameForSpeech(name) {
  const raw = normalizeSpaces(name)
    ...
    .replace(/\s+(?:knowledge\s*base|kb)\s*$/i, "")
    .trim();
  if (!raw) return "";
  if (
    /^(?:business|company|organization|tenant|knowledge\s*base|kb|test|unknown|none|null|n\/a|na)$/i.test(
      raw,
    )
  )
    return "";
...
function comparableSpeechLabel(value) {
...
function sameSpeechLabel(a, b) {
  const left = comparableSpeechLabel(a);
  const right = comparableSpeechLabel(b);
...
function buildCallerIdentityPhrase({
  ...
  if (agent && org) {
    if (sameSpeechLabel(agent, org)) return agent;
    return `${agent} from ${org}`;
  }
...
function repairOutboundAssistantText(text, context = {}) {
  ...
  output = output.replace(
    /\b(this\s+is\s+)([^,.!?]{1,60}?)\s+from\s+\2(?=\s*[,.!?]|\s|$)/gi,
    "$1$2",
  );
