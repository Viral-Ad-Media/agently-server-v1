...
function interpretOutboundPurposeForSpeech(text) {
  ...
  const hasProductInterest =
    /interest|interested|purchase|purchasing|buy|order|product|products|supplement|supplements|chasing/.test(
      lower,
    );

  if (
    /ask\s+for\s+their\s+well[- ]?being/i.test(lower) ||
    /interest\s+in\s+our\s+(?:chasing|purchasing|products?)/i.test(lower)
  ) {
    return "a quick wellness check-in to see how you're doing and whether any of our products might be useful for you";
  }

  if (hasWellness && hasProductInterest) {
    return "a quick wellness check-in to see how you're doing and whether any of our products might be useful for you";
  }
...
function buildOutboundGreeting({
  ...
  const identity = buildCallerIdentityPhrase({ agentName, organizationName });
  const reason = outboundGreetingReasonClause(callPurpose);
  const prefix = cleanName ? `Hello ${cleanName},` : "Hello,";
  if (identity === "the team") {
    return `${prefix} I'm calling to follow up briefly. Do you have a quick moment to talk?`;
  }
...
function callEndingRules({ recipientName = "" } = {}) {
  ...
  return [
    "CALL ENDING RULE:",
    "- Once the call purpose is fulfilled, ask only one closing confirmation such as: 'Will that be all for today?' or 'Is there anything else I can help you with?'",
    "- If the caller confirms, says okay/alright/sure/thanks/bye/goodbye, or says there is nothing else, do not reopen the conversation.",
    cleanName
    ...
    "- After the final goodbye, do not ask whether they want to leave a message. Do not ask new questions. The system will end the call.",
