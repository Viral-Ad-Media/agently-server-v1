...
async function insertUsageEvent(event) {
  ...
  try {
    ...
    console.warn(
      "[usage-ledger] record_billing_usage_event missing; falling back to direct insert",
    );
  }
...
function isAutoWalletChargeEnabled() {
  return ["1", "true", "yes", "on"].includes(
    String(process.env.BILLING_AUTO_CHARGE_WALLET || "").toLowerCase(),
  );
}
...
async function logOpenAIUsage({
  ...
  const componentEntries = Object.entries(normalized).filter(
  ...
  if (!componentEntries.length) {
    return insertUsageEvent({
      ...common,
      eventType: `${eventType}_usage_missing`,
      externalId: `${externalId || stableKey([organizationId, service, eventType, nowIso()])}:usage_missing`,
      quantity: 0,
      ...
      metadata: {
        ...
        reason: "OpenAI response did not include usable token component usage.",
      },
    });
  }

  const rows = [];
  for (const [component, quantity] of componentEntries) {
    rows.push(
      await insertUsageEvent({
        ...common,
        eventType: `${eventType}_${component}`,
        externalId: `${externalId || stableKey([organizationId, service, eventType])}:${component}`,
        ...
        metadata: {
          ...
          token_components: normalized,
          original_event_type: eventType,
          cost_status: "exact_units_recorded_cost_requires_rate_card",
        },
      }),
    );
  }
  ...
  if (estimatedCostUsd != null) {
    rows.push(
      await insertUsageEvent({
        ...common,
        eventType: `${eventType}_provider_reported_total_cost`,
        externalId: `${externalId || stableKey([organizationId, service, eventType])}:provider_cost`,
        idempotencyKey: stableKey([
          "openai",
          service,
          eventType,
          externalId,
          "provider_reported_total_cost",
          organizationId,
          callId,
          chatbotId,
          voiceAgentId,
        ...
        metadata: {
          ...
          token_components: normalized,
          original_event_type: eventType,
          cost_status: "provider_reported_total_cost",
        },
      }),
    );
  }
...
async function logRailwayRuntimeUsage({
  ...
  return insertUsageEvent({
    ...
    provider: "railway",
    service: "runtime",
    eventType: service || "websocket_runtime",
...
async function estimateTenantStorageBytes(organizationId) {
  ...
  const tables = [
    ...
    },
    {
      ...
      select:
        "id,knowledge_base_id,title,url,status,metadata,created_at,updated_at",
      orgColumn: "organization_id",
    },
    {
