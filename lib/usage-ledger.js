...
const { getMinimumTopUpUsd } = require("./billing-settings");
...
function inferProviderResourceTypesFromPayload(payload) {
  ...
  if (provider === "twilio") {
    ...
    add("instance_id", metadata.instance_id);
...
async function ensureWalletRow(sb, organizationId, balanceUsd = 0) {
  ...
  const payload = {
    ...
    minimum_recharge_usd: await getMinimumTopUpUsd(sb),
    status: "active",
    updated_at: nowIso(),
  };
  const { data, error } = await sb
...
/**
 * Per-token cost for an OpenAI usage component.
 *
 * Prefers an explicit OPENAI_RATE_CARD_JSON entry matching this service and
 * event type, so the vendor-rate sync stays the source of truth. Falls back to
 * published gpt-4o-mini pricing, which is what the chatbot actually runs on —
 * a wrong-but-close cost still bills roughly right, whereas null bills zero.
 * Both overridable by env without a deploy.
 */
let _openAiChatRates = null;
function openAiTokenUnitCostUsd(service, component, model) {
  if (!_openAiChatRates) {
    const rates = {};
    try {
      ...
      if (raw) {
        for (const row of JSON.parse(raw)) {
          ...
          const key = `${String(row.service || "").toLowerCase()}|${String(row.eventType || row.event_type || "").toLowerCase()}`;
          const cost = Number(row.unitCostUsd ?? row.unit_cost_usd);
          if (Number.isFinite(cost)) rates[key] = cost;
        }
      }
    } catch (err) {
      console.warn("[usage-ledger] OPENAI_RATE_CARD_JSON did not parse:", err.message);
    }
    _openAiChatRates = rates;
  }

  const suffix = String(component?.eventSuffix || "").toLowerCase();
  const exact = _openAiChatRates[`${String(service || "").toLowerCase()}|${suffix}`];
  if (Number.isFinite(exact) && exact > 0) return exact;

  const isOutput = suffix.includes("output");
  const isCached = suffix.includes("cached");
  const fallback = isCached
    ? Number(process.env.OPENAI_CHAT_CACHED_INPUT_COST_PER_TOKEN || 0.000000075)
    : isOutput
      ? Number(process.env.OPENAI_CHAT_OUTPUT_COST_PER_TOKEN || 0.0000006)
      : Number(process.env.OPENAI_CHAT_INPUT_COST_PER_TOKEN || 0.00000015);
  void model;
  ...
  for (const component of componentRows) {
    rows.push(
      await insertUsageEvent({
        ...
        // Without a unit cost the event stores unit_cost_usd = null, internal
        // cost resolves to 0, and the target-margin rule prices it 0/0.30 =
        // $0.00 — which is why chatbot replies were effectively free while
        // still costing real OpenAI money. Same failure the Railway runtime
        // meter had.
        unitCostUsd: openAiTokenUnitCostUsd(service, component, model),
...
// Seconds of live websocket work per org. Feeds the shared-infrastructure
// apportionment in lib/usage-billing-engine.js; see the billable note below.
async function logRuntimeUsage({
  organizationId,
  userId,
  seconds,
  estimatedCostUsd,
  ...
  return insertUsageEvent({
    ...
    estimatedCostUsd: null,
    // NOT BILLABLE. We left Railway, so there is no Railway invoice to recover
    // and charging a tenant a Railway rate would bill them for a vendor we do
    // not pay. The measurement is still worth keeping and is still recorded:
    // seconds of live websocket work per org is exactly the driver that
    // lib/usage-billing-engine.js apportions the AWS Lightsail bill by. That
    // box is now recovered there, from its real fixed monthly bundle price,
    // rather than from a per-second rate card for the wrong vendor.
    billable: false,
    metadata: {
      ...(metadata || {}),
      runtime_vendor: "aws_lightsail",
      billing_note:
        "Runtime seconds are a shared-infrastructure cost driver, not a directly billable provider cost. See lib/usage-billing-engine.js.",
      superseded_estimated_cost_usd:
        estimatedCostUsd == null ? null : safeNumber(estimatedCostUsd),
    },
  });
}
...
async function logKnowledgeSyncUsage({
  ...
  // pages_attempted / pages_failed are NOT billable: they share unit "pages"
  // with pages_scraped, and apply_billing_rate()'s fallback used to price any
  // event by provider+unit alone when no exact match existed — silently
  // double/triple-charging one scrape as attempted + scraped + failed. Kept
  // as metadata on sync_attempt for visibility instead of separate events.
  await add("knowledge_base", "scrape_sync", "sync_attempt", "sync", 1, {
    duration_ms: durationMs || null,
    pages_attempted: safeNumber(pagesAttempted),
    pages_failed: safeNumber(pagesFailed),
  });
  await add(
    "knowledge_base",
    "scrape_sync",
...
async function buildTenantUsageReport({
  ...
  for (const row of usageRows || []) {
    ...
    // "railway" is kept here only so historical rows still count toward the
    // runtime total in reports. Nothing writes railway rows any more.
    if ((provider === "aws" || provider === "railway") && unit === "seconds")
      item.runtimeSeconds += qty;
  }
...
module.exports = {
  insertUsageEvent,
  isAutoWalletChargeEnabled,
  // PATCH: required by lib/wallet-settlement.js. Settlement must post debits
  // through the exact same path as live usage or balances will drift.
