...
async function fetchDirect(url) {
  // safeFetch validates the resolved address on every hop and pins the socket
  // to it. The tenant supplies this URL, so a plain fetch here was a way to
  // read anything reachable from inside the container. See lib/safe-fetch.js.
...
async function fetchViaJina(url) {
  // The host here is fixed and public, so this is not the SSRF path the other
  // two are — Jina fetches the tenant's URL from ITS network, not ours. Routed
  // through safeFetch anyway for one reason worth having: the response size
  // cap. A reader API returning an enormous body should not be able to sit in
  // this process's memory unbounded.
  const jinaUrl = `https://r.jina.ai/${url}`;
  const { text } = await safeFetch(jinaUrl, {
    timeoutMs: 12000,
    accept: "text/plain",
