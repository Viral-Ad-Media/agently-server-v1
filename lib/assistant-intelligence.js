  ...
  const rows = await safeQuery(
    ...
    () =>
      db
        ...
        .select(
          "id,name,business_name,description,industry,primary_url,domain,is_primary,status,sync_status,last_synced_at",
        )
...
function knowledgeIdentityWebsite(context) {
  const base = primaryKnowledgeBase(context);
  return cleanText(
    base?.primary_url || base?.domain || context?.organization?.website || "",
  );
}
...
async function loadVoiceAgentContext(agentId, query = "", options = {}) {
  ...
  for (const bot of linkedChatbots || []) {
    if (bot.custom_prompt && knowledgeBaseIds.length && bot.knowledge_base_id && knowledgeBaseIds.includes(bot.knowledge_base_id)) {
...
- If the user asks what the business does, summarize only from the scoped selected Knowledge Base FAQs and chunks.
...
function buildAssistantPrompt({
  ...
  const displayIndustry = kb?.industry || org.industry || "";
  const orgBlock =
