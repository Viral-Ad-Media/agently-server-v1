...
const MAX_CONTEXT_CHARS = Number(process.env.ASSISTANT_MAX_CONTEXT_CHARS || 26000);

const STOPWORDS = new Set([
  "a","an","and","are","as","at","be","but","by","can","could","do","for","from","go","give","get","help","how","i","in","is","it","me","my","need","of","on","or","our","please","show","take","that","the","them","there","this","to","want","what","where","which","with","you","your","about","some","tell","have","has","into","onto","than","then","was","were","will","would"
]);
...
function normalizeChunk(c) {
  ...
  const inlineUrls = extractUrls(content).map((url) => ({ label: labelFromUrl(url), url }));
  const sourceLink = sourceUrl ? [{ label: sourceTitle || labelFromUrl(sourceUrl), url: sourceUrl }] : [];
  const links = uniqueBy([...sourceLink, ...mdLinks, ...inlineUrls].filter((x) => x.url), (x) => x.url);
...
function rankChunks(message, chunks, limit = MAX_CHUNKS) {
  ...
  const scored = (chunks || [])
    .map((chunk) => ({
      ...chunk,
      _score: scoreText(tokens, `${chunk.source_title || ""} ${chunk.content || ""} ${(chunk.links || []).map((l) => `${l.label} ${l.url}`).join(" ")}`, chunk.source_url || ""),
    }))
    .sort((a, b) => b._score - a._score);
  ...
  return scored.slice(0, Math.min(8, limit)).map(({ _score, ...chunk }) => chunk);
}
...
async function loadOrganization(organizationId) {
  ...
  const data = await safeQuery("organization", () =>
    getSupabase().from("organizations").select("id,name,industry,website,location,phone_number,timezone").eq("id", organizationId).maybeSingle(),
    null,
  );
  return data || null;
}

async function getOrgFallbackChunks(organizationId, excludeChatbotId, excludeAgentId, limit = 80) {
  if (!organizationId) return [];
  let rows = await safeQuery("organization knowledge chunks", () =>
    getSupabase()
      .from("knowledge_chunks")
      .select("source_url,source_title,content,chunk_index,chatbot_id,voice_agent_id")
      .eq("organization_id", organizationId)
      .order("chunk_index", { ascending: true })
      .limit(limit),
  );
  // Keep fallback broad, but prefer rows not already selected by exact chatbot/agent.
  return (rows || []).filter((r) => r.chatbot_id !== excludeChatbotId || r.voice_agent_id !== excludeAgentId);
}
...
async function loadChatbotContext(chatbotId) {
  ...
  if (chatbot.voice_agent_id) {
    linkedAgent = await safeQuery("linked voice agent", () =>
      db.from("voice_agents").select("id,name,greeting,tone,business_hours,escalation_phone,data_capture_fields,rules,call_purposes,direction,language").eq("id", chatbot.voice_agent_id).maybeSingle(),
      null,
    );
    ...
    linkedAgentChunks = await safeQuery("linked voice chunks", () =>
      db.from("knowledge_chunks").select("source_url,source_title,content,chunk_index").eq("voice_agent_id", chatbot.voice_agent_id).limit(80),
    );
  }

  const orgFallback = chunks.length || linkedAgentChunks.length ? [] : await getOrgFallbackChunks(chatbot.organization_id, chatbotId, chatbot.voice_agent_id, 100);

  const allChunks = uniqueBy([...chunks, ...linkedAgentChunks, ...orgFallback].map(normalizeChunk).filter(Boolean), (c) => `${c.source_url}|${c.content.slice(0, 140)}`);
  const allFaqs = uniqueBy([...chatbotFaqs, ...linkedAgentFaqs.map(normalizeFaq).filter(Boolean)], (f) => `${f.question}|${f.answer}`).slice(0, MAX_FAQS);
  ...
  return {
    ...
    faqs: allFaqs,
    chunks: allChunks,
    stats: { faqs: allFaqs.length, chunks: allChunks.length, links: collectLinks(allChunks).length },
  };
}

async function loadVoiceAgentContext(agentId) {
  const db = getSupabase();
  const { data: agent, error } = await db.from("voice_agents").select("*").eq("id", agentId).maybeSingle();
  ...
  const [agentFaqs, agentChunks, linkedChatbots] = await Promise.all([
    ...
    safeQuery("voice chunks", () => db.from("knowledge_chunks").select("source_url,source_title,content,chunk_index").eq("voice_agent_id", agentId).limit(120)),
    safeQuery("linked chatbots", () => db.from("chatbots").select("id,custom_prompt,faqs,name,header_title,welcome_message").eq("voice_agent_id", agentId).limit(10)),
  ]);
  ...
  for (const bot of linkedChatbots || []) {
    ...
    const chunks = await safeQuery(`chatbot chunks ${bot.id}`, () =>
      db.from("knowledge_chunks").select("source_url,source_title,content,chunk_index").eq("chatbot_id", bot.id).limit(80),
    );
    chatbotChunks.push(...chunks);
  }

  const orgFallback = agentChunks.length || chatbotChunks.length ? [] : await getOrgFallbackChunks(agent.organization_id, null, agentId, 120);
  const allChunks = uniqueBy([...agentChunks, ...chatbotChunks, ...orgFallback].map(normalizeChunk).filter(Boolean), (c) => `${c.source_url}|${c.content.slice(0, 140)}`);
  const allFaqs = uniqueBy([...agentFaqs.map(normalizeFaq).filter(Boolean), ...chatbotFaqs], (f) => `${f.question}|${f.answer}`).slice(0, MAX_FAQS);
...
function collectLinks(chunks = []) {
  return uniqueBy(chunks.flatMap((c) => c.links || []), (l) => l.url).slice(0, 80);
}
...
function buildKnowledgeBlock({ faqs = [], chunks = [] }) {
  ...
  const parts = [];
  if (faqs.length) {
    const faqText = faqs.map((f, i) => `${i + 1}. Q: ${f.question}\n   A: ${f.answer}`).join("\n");
  ...
  if (chunks.length && used < MAX_CONTEXT_CHARS) {
    ...
    for (let i = 0; i < chunks.length; i++) {
      const c = chunks[i];
      const links = (c.links || []).slice(0, 10).map((l) => `[${l.label || labelFromUrl(l.url)}](${l.url})`).join(" | ");
      const title = c.source_title || (c.source_url ? labelFromUrl(c.source_url) : `Knowledge ${i + 1}`);
    ...
    if (chunkLines.length) parts.push(`SCOPED WEBSITE KNOWLEDGE AND LINKS:\n${chunkLines.join("\n\n")}`);
  }

  const linkCatalog = collectLinks(chunks);
  if (linkCatalog.length && used < MAX_CONTEXT_CHARS) {
    parts.push(`AVAILABLE LINK CATALOG:\n${linkCatalog.slice(0, 40).map((l, i) => `${i + 1}. [${l.label || labelFromUrl(l.url)}](${l.url})`).join("\n")}`);
  }
  return parts.filter(Boolean).join("\n\n");
}

function businessName(context) {
  return cleanText(context?.organization?.name || context?.entity?.header_title || context?.entity?.name || context?.linkedAgent?.name || "this business");
}

function baseBehaviorInstructions({ context, mode = "text", direction = "inbound", languageName = "English" }) {
  const entity = context.entity || {};
  const org = context.organization || {};
  const isVoice = mode === "voice";
  const captureFields = asArray(entity.data_capture_fields).length ? asArray(entity.data_capture_fields).join(", ") : "name, phone, email, reason";
  ...
  const noKnowledgeLine = context.stats?.chunks || context.stats?.faqs
...
function buildAssistantPrompt({ context, message = "", mode = "text", direction = "inbound", languageName = "English" }) {
  ...
  const fallbackFaqs = relevantFaqs.length ? relevantFaqs : (context.faqs || []).slice(0, 12);
  const fallbackChunks = relevantChunks.length ? relevantChunks : (context.chunks || []).slice(0, Math.min(10, MAX_CHUNKS));
  const org = context.organization || {};
  const orgBlock = org.id ? `ORGANIZATION DETAILS:\nName: ${org.name || ""}\nIndustry: ${org.industry || ""}\nWebsite: ${org.website || ""}\nLocation: ${org.location || ""}\nPhone: ${org.phone_number || ""}` : "";
  const greeting = context.entity?.greeting ? `CONFIGURED GREETING:\n${context.entity.greeting}` : "";

  return [
    context.customPrompt ? `CUSTOM PROMPT FROM BUSINESS OWNER:\n${context.customPrompt}` : "",
    orgBlock,
    greeting,
...
async function generateGroundedChatResponse({ message, history = [], chatbotId, languageName = "English" }) {
  ...
  const completion = await openai.chat.completions.create({
    ...
    messages: [
      ...
      ...history.slice(-10).map((m) => ({ role: m.role === "model" ? "assistant" : "user", content: m.text || m.content || "" })),
      { role: "user", content: message },
    ],
  });

  const fallback = context.stats?.chunks || context.stats?.faqs
  ...
  return { response: cleanAssistantResponse(completion.choices[0]?.message?.content || fallback), context };
}

function looksUnanswered(text) {
  const lower = String(text || "").toLowerCase();
  return ["could not find", "don't have", "do not have", "not available", "not listed", "not in the knowledge base", "someone can follow up", "leave your details", "take your details"].some((p) => lower.includes(p));
}
