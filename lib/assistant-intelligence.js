...
async function loadChatbotContext(chatbotId) {
  ...
  const chunks = knowledgeBaseIds.length
    ? await safeQuery("chatbot scoped knowledge chunks", () =>
        db
          .from("knowledge_chunks")
          .select(
            "source_url,source_title,content,chunk_index,knowledge_base_id,knowledge_source_id",
          )
  ...
  if (chatbot.voice_agent_id) {
    ...
    linkedAgentFaqs = knowledgeBaseIds.length
      ? await safeQuery("linked voice scoped faqs", () =>
          db
            .from("faqs")
            .select("question,answer,knowledge_base_id")
            .eq("organization_id", chatbot.organization_id)
            .eq("voice_agent_id", chatbot.voice_agent_id)
            .in("knowledge_base_id", knowledgeBaseIds)
            .limit(MAX_FAQS),
        )
      : await safeQuery("linked voice faqs", () =>
          db
            .from("faqs")
    ...
    linkedAgentChunks = knowledgeBaseIds.length
      ? []
      : await safeQuery("linked voice chunks", () =>
          db
            .from("knowledge_chunks")
            .select("source_url,source_title,content,chunk_index")
            .eq("voice_agent_id", chatbot.voice_agent_id)
