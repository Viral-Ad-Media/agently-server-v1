...
const MAX_PRODUCTS = Number(process.env.ASSISTANT_MAX_PRODUCTS || 80);
...
function normalizeChunk(c) {
  ...
  return {
    ...
    links,
    content_kind: cleanText(
      c.content_kind ||
        c.kind ||
        c.metadata?.contentKind ||
        c.metadata?.kind ||
        "page",
    ).toLowerCase(),
...
function normalizeProduct(product) {
  ...
  const name = cleanText(product.name || product.title || product.product_name);
  const url = safeHttpUrl(
    product.url || product.product_url || product.source_url,
  );
  ...
  return {
    ...
    description: cleanText(
      product.description || product.summary || product.content,
    ),
    price: product.price == null ? null : Number(product.price),
    price_text: cleanText(product.price_text || product.priceText),
    ...
    image_url: safeHttpUrl(product.image_url || product.imageUrl),
    knowledge_base_id: product.knowledge_base_id || null,
    knowledge_source_id: product.knowledge_source_id || null,
    raw_source: cleanText(
      product.raw_source || product.rawSource || "knowledge",
    ),
  };
}

function isProductIntent(message) {
  return /\b(product|products|item|items|buy|purchase|order|shop|shopping|price|cost|available|availability|stock|checkout|cart|recommend|recommendation|sell|selling)\b/i.test(
    String(message || ""),
  );
}

function rankProducts(message, products, limit = 12) {
  ...
  if (
    ...
    (isProductIntent(message) &&
      tokens.every((token) =>
        /^(product|products|item|items|buy|order|shop|price|cost)$/.test(token),
      ))
  ) {
  ...
  const scored = normalized
    .map((product) => ({
      ...
      _score:
        scoreText(
          tokens,
          `${product.name} ${product.brand} ${product.sku} ${product.description} ${product.price_text} ${product.availability}`,
          product.url,
        ) +
        (cleanText(message).toLowerCase().includes(product.name.toLowerCase())
          ? 8
          : 0),
    }))
  ...
  const chosen = relevant.length
    ...
    : isProductIntent(message)
      ...
      : [];
  return chosen.map(({ _score, ...product }) => product);
}

function deriveProductsFromChunks(chunks = []) {
  return uniqueBy(
    chunks
      .filter((chunk) =>
        ["product", "product_fact"].includes(
          String(chunk.content_kind || "").toLowerCase(),
        ),
      )
      .map((chunk) => {
        const url = safeHttpUrl(chunk.source_url);
        if (!url) return null;
        const priceMatch = String(chunk.content || "").match(
          /(?:USD|GBP|EUR|NGN|[$£€₦])\s?\d[\d,.]*/i,
        );
        return normalizeProduct({
          name: chunk.source_title || labelFromUrl(url),
          url,
          description: truncate(chunk.content, 520),
          price_text: priceMatch?.[0] || "",
          raw_source: `knowledge_chunk:${chunk.content_kind || "product"}`,
        });
      })
      .filter(Boolean),
    (product) => product.url,
...
async function loadProductsByKbIds(db, organizationId, knowledgeBaseIds = []) {
  const ids = Array.isArray(knowledgeBaseIds)
    ? knowledgeBaseIds.filter(Boolean)
    : [];
  if (!organizationId || !ids.length) return [];
  const rows = await safeQuery(
    "scoped scraped products",
    () =>
      db
        .from("scraped_products")
        .select(
          "id,name,url,description,price,price_text,currency,availability,brand,sku,image_url,knowledge_base_id,knowledge_source_id,raw_source",
        )
  ...
  return uniqueBy(
    (rows || []).map(normalizeProduct).filter(Boolean),
    (product) => product.url,
  );
}
...
async function loadChatbotContext(chatbotId, query = "") {
  ...
  const storedProducts = await loadProductsByKbIds(
    db,
    chatbot.organization_id,
    knowledgeBaseIds,
  );
  const allProducts = uniqueBy(
    [...storedProducts, ...deriveProductsFromChunks(allChunks)],
    (product) => product.url,
  ...
  return {
    ...
    stats: {
      ...
      // A chatbot pointed at an empty knowledge base cannot answer anything,
      // and from outside that is indistinguishable from a broken model — it
      // says "I do not know" to everything. One deployed bot was attached to a
      // knowledge base with zero chunks while the content it needed sat in a
      // different organization. Surfacing it means the operator is told,
      // instead of concluding the agent got dumber.
      knowledgeEmpty: allChunks.length === 0 && allFaqs.length === 0,
      links: collectLinks(allChunks).length + allProducts.length,
...
function buildKnowledgeBlock({ faqs = [], chunks = [], products = [] }) {
  let used = 0;
  const parts = [];
  if (faqs.length) {
    const faqText = faqs
  ...
  if (products.length && used < MAX_CONTEXT_CHARS) {
    const productLines = products.slice(0, 24).map((product, index) => {
      const details = [
        product.price_text ||
      ...
      return `${index + 1}. [${product.name}](${product.url})${details.length ? ` — ${details.join(" | ")}` : ""}${product.description ? `\n   ${truncate(product.description, 420)}` : ""}`;
    });
    const productText = productLines.join("\n");
    if (used + productText.length <= MAX_CONTEXT_CHARS) {
      used += productText.length;
      parts.push(
        `SCOPED PRODUCT CATALOG (direct product URLs):\n${productText}`,
      );
    }
  }
...
function buildAssistantPrompt({
  ...
  const relevantProducts = rankProducts(message, context.products || [], 16);
...
function ensureProductLinks({ message, response, products = [], chunks = [] }) {
  const cleaned = cleanAssistantResponse(response);
  if (!isProductIntent(message)) return cleaned;

  let matches = rankProducts(message, products, 5).filter(
    (product) => product.url,
  );

  // Poorly built shops often expose no Product schema and use paths like
  // /jug or /phone. For a specific product request, use the strongest scoped
  // source page as a safe direct-link fallback rather than falling back to the
  // homepage or inventing a conventional /product/ URL.
  if (!matches.length) {
    const specificTokens = tokenize(message).filter(
      (token) =>
        !/^(product|products|item|items|buy|purchase|order|shop|shopping|price|cost|available|availability|stock|checkout|cart|recommend|recommendation|sell|selling)$/.test(
          token,
        ),
    );
      ...
      matches = uniqueBy(
        (chunks || [])
          .map((chunk) => {
            ...
            if (
              !url ||
              /\/(about|contact|privacy|terms|blog|news)(\/|$)/i.test(url)
            ) {
              return null;
            }
  ...
  const missing = matches.filter(
    (product) => !lower.includes(product.url.toLowerCase()),
  );
  if (!missing.length) return cleaned;

  const direct =
    missing.length === 1
      ? `\n\n[View or purchase ${missing[0].name}](${missing[0].url})${missing[0].price_text ? ` — ${missing[0].price_text}` : ""}`
      : `\n\n**Direct product links**\n${missing
          .map(
            (product, index) =>
              `${index + 1}. [${product.name}](${product.url})${product.price_text ? ` — ${product.price_text}` : ""}`,
          )
          .join("\n")}`;
  return `${cleaned}${direct}`.trim();
}
...
async function generateGroundedChatResponse({
  ...
  const completion = await openai.chat.completions.create({
    model: CHAT_MODEL,
    temperature: 0.15,
    // The deployed widget was capped at 650 while the in-app preview allows
    // 1200, so the same question produced a visibly shorter, clipped answer in
    // production — part of why the live bot reads as less capable than the
    // preview. Matched to the preview so both give the same answer.
    max_tokens: 1200,
    messages: [
  ...
  const fallback =
    context.stats?.chunks || context.stats?.faqs || context.stats?.products
  ...
  // When the model runs out of room it stops mid-sentence, which reads as the
  // bot glitching. Trim back to the last complete sentence and offer to
  // continue, so a long answer ends deliberately instead of being severed.
  // Loud, once per reply: a bot answering from an empty knowledge base looks
  // broken to everyone except the logs.
  if (context.stats?.knowledgeEmpty) {
    console.warn(
      "[chatbot] answering with an EMPTY knowledge base — it cannot answer anything",
      {
        chatbotId,
        knowledgeBaseIds: context.knowledgeBaseIds || [],
        hint: "The attached knowledge base has no chunks and no FAQs. Attach one with content, or scan pages into it.",
      },
    );
  }

  const choice = completion.choices[0];
  let text = choice?.message?.content || fallback;
  if (choice?.finish_reason === "length" && text) {
    ...
    // Only trim if it leaves a substantial answer — otherwise keep everything
    // rather than throw away most of a short reply on one stray delimiter.
    if (lastStop > text.length * 0.5) text = text.slice(0, lastStop + 1);
    text = `${text.trim()}\n\nThere's more on this — say "continue" and I'll carry on.`;
  }

  return {
    response: ensureProductLinks({
      message,
