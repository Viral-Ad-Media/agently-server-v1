...
async function stripeApiRequest(
  ...
  // A bare fetch() has no request timeout in Node — undici will wait on a
  // stalled socket for minutes. The browser gives up after 15s and shows
  // "Agently could not reach its data service in time", so an untimed Stripe
  // call on a request path turns a Stripe hiccup into what looks to the user
  // like our API being down. Cap it below the client's own deadline so the
  // failure is ours to report rather than theirs to guess at.
  const timeoutMs = Math.min(
    Math.max(Number(process.env.STRIPE_TIMEOUT_MS) || 8000, 1000),
    30000,
  );
  ...
  try {
    ...
    const error = new Error(
      timedOut
        ? `Stripe did not respond within ${timeoutMs}ms. Please retry.`
        : "Agently could not reach Stripe. Please retry.",
    );
    error.status = 503;
    error.code = timedOut ? "STRIPE_TIMEOUT" : "STRIPE_NETWORK_ERROR";
    error.cause = cause;
    throw error;
  } finally {
    clearTimeout(timer);
...
async function createWalletTopUpCheckout({ organizationId, user, amountUsd }) {
  /*
   * Checkout needs the secret key. It does NOT need the webhook secret: when a
   * pushed event cannot be verified, getWalletTopUpStatus still recovers the
   * credit by polling the session. Demanding both here is what made a working
   * Stripe integration report itself as unconfigured to tenants.
   */
  if (!stripeCheckoutConfigured()) {
    const error = new Error(
      "Stripe top-ups require STRIPE_SECRET_KEY before customer payments can be accepted.",
    );
    error.status = 503;
    error.code = "STRIPE_NOT_CONFIGURED";
    throw error;
  ...
  try {
    session = await stripeApiRequest("/checkout/sessions", {
      ...
      fields: {
        ...
        /*
         * Save the card for later. Without this the tenant pays, gets their
         * credit, and STILL has no payment method on file — so the activation
         * gate stays shut and every future top-up needs another redirect.
         */
...
async function processStripeWebhook(event) {
  ...
  try {
    ...
    if (
      ...
      /*
       * D5 — the one-click saved-card flow credits HERE and nowhere else.
       * Only PaymentIntents we created for a wallet top-up carry top_up_id, so
       * unrelated intents fall through as ignored. Event-id dedupe above plus
       * the credited-row check inside make a replay a no-op.
       */
      result = await creditSucceededPaymentIntent(object, event.id);
...
/* ══════════════════════════════════════════════════════════════════════════
 * CARD ON FILE  (D2)  +  ONE-CLICK TOP-UP  (D3)  +  DOUBLE-CHARGE GUARD (D4)
 * ══════════════════════════════════════════════════════════════════════════ */

/**
 * D2 — a SetupIntent saves a card WITHOUT charging it.
 *
 * The gate exists so a tenant cannot provision a recurring cost and walk away;
 * it does not exist to take money up front. So the card is collected and
 * stored, nothing is captured, and the tenant's free credit stays spendable.
 */
async function createCardSetupIntent({ organizationId, user }) {
  if (!stripeCheckoutConfigured()) {
    const error = new Error(
      "Card setup requires STRIPE_SECRET_KEY before a payment method can be saved.",
    );
    error.status = 503;
    error.code = "STRIPE_NOT_CONFIGURED";
    throw error;
  ...
  const intent = await stripeApiRequest("/setup_intents", {
    method: "POST",
    fields: {
      customer: stripeCustomerId,
      usage: "off_session", // so it can be charged later without the user present
      "payment_method_types[0]": "card",
      "metadata[organization_id]": organizationId,
...
/** Cards Stripe currently holds for this org. Empty array = gate stays shut. */
async function listSavedCards(db, organizationId) {
  ...
  const result = await stripeApiRequest(
    `/payment_methods?customer=${encodeURIComponent(row.stripe_customer_id)}&type=card&limit=10`,
    { method: "GET" },
  );
...
/**
 * D4 — per-tenant in-flight lock.
 *
 * Client-side disabling is not enough: a double submit can arrive on two
 * connections before React re-renders, and a retried fetch bypasses the UI
 * entirely. This is the second layer; the Stripe idempotency key below is the
 * third and is what actually makes a duplicate request harmless.
 */
const chargeLocks = new Map();
const LOCK_TTL_MS = 30000;

function acquireChargeLock(organizationId) {
  const now = Date.now();
  const held = chargeLocks.get(organizationId);
  if (held && now - held < LOCK_TTL_MS) return false;
  chargeLocks.set(organizationId, now);
  return true;
}

function releaseChargeLock(organizationId) {
  chargeLocks.delete(organizationId);
}

/**
 * D3 — charge a saved card directly. No checkout redirect for returning
 * top-ups.
 *
 * D5 note: this does NOT credit the wallet. It creates the top-up row and the
 * PaymentIntent; the wallet only moves when payment_intent.succeeded arrives
 * on the webhook, deduped by Stripe event id. A card that later fails or is
 * disputed therefore credits nothing.
 */
async function chargeSavedCard({ organizationId, user, amountUsd, paymentMethodId }) {
  if (!stripeCheckoutConfigured()) {
  ...
  if (!acquireChargeLock(organizationId)) {
    const error = new Error(
      "A top-up is already being processed for this workspace. Give it a moment before trying again.",
    );
    error.status = 409;
    error.code = "TOP_UP_IN_PROGRESS";
    throw error;
  }
  ...
  try {
    const db = getSupabase();
    const settings = await getBillingPlatformSettings(db);
    const minimumTopUpUsd = Number(settings.minimumTopUpUsd || 10);
    const amount = roundMoney(safeNumber(amountUsd, 0));

    if (amount < minimumTopUpUsd) {
      const error = new Error(`Minimum top-up is $${minimumTopUpUsd.toFixed(2)}.`);
      error.status = 400;
      error.code = "TOP_UP_BELOW_MINIMUM";
      throw error;
    }
    if (amount > maxTopUpUsd()) {
      const error = new Error(`Maximum top-up is $${maxTopUpUsd().toFixed(2)}.`);
      error.status = 400;
      error.code = "TOP_UP_ABOVE_MAXIMUM";
      throw error;
    }
    ...
    if (!cards.length) {
      const error = new Error("No saved card on file. Add a payment method first.");
      error.status = 402;
      error.code = "CARD_REQUIRED";
      throw error;
    }
    const useCard =
      cards.find((c) => c.id === paymentMethodId)?.id || cards[0].id;
    ...
    let intent;
    try {
      intent = await stripeApiRequest("/payment_intents", {
        method: "POST",
        /*
         * Keyed on our own top-up id: a retried or double-submitted request
         * that reaches Stripe twice resolves to the SAME PaymentIntent rather
         * than charging the card again.
         */
        idempotencyKey: `agently-wallet-charge-${topUpId}`,
        fields: {
          amount: Math.round(amount * 100),
          currency: "usd",
          customer: customerRow?.stripe_customer_id,
          payment_method: useCard,
          off_session: "true",
          confirm: "true",
          description: "Agently usage credit",
          "metadata[purpose]": "wallet_top_up",
          "metadata[top_up_id]": topUpId,
          "metadata[organization_id]": organizationId,
        },
      ...
      await db
        ...
        .update({
          status: "payment_failed",
          failure_message: cleanString(stripeError?.message, 500),
          updated_at: new Date().toISOString(),
        })
        .eq("id", topUpId);
      throw stripeError;
    ...
    await db
      ...
      .update({
        ...
        status: intent.status === "succeeded" ? "awaiting_webhook" : intent.status,
        updated_at: new Date().toISOString(),
      })
      .eq("id", topUpId);
    ...
    return {
      ...
      amountUsd: amount,
      /*
       * Deliberately reports "pending" even when Stripe says succeeded: the
       * balance has not moved yet and will not until the webhook lands.
       * Telling the UI otherwise is how optimistic crediting starts.
       */
      credited: false,
    };
  } finally {
    releaseChargeLock(organizationId);
  }
}

/**
 * D5 — credit a PaymentIntent-based top-up. Called only from the webhook.
 * Returns { credited:false, duplicate:true } if the row was already credited,
 * so a replayed event cannot double-credit even if event dedupe were bypassed.
 */
async function creditSucceededPaymentIntent(paymentIntent, eventId = null) {
  const db = getSupabase();
  ...
  const { data: topUp, error } = await db
    .from("billing_wallet_topups")
    .select("id,organization_id,amount_usd,status,wallet_transaction_id")
    .eq("id", topUpId)
    .maybeSingle();
  if (error) throw error;
  if (!topUp) return { credited: false, ignored: true };

  if (topUp.wallet_transaction_id && topUp.status === "credited") {
    return { credited: true, duplicate: true, topUpId };
  }

  const { error: rpcError } = await db.rpc("billing_credit_wallet_from_stripe", {
    p_top_up_id: topUp.id,
    p_organization_id: topUp.organization_id,
    p_amount_usd: Number(topUp.amount_usd || 0),
    ...
    p_event_id: eventId,
    p_metadata: { flow: "saved_card", stripe_status: paymentIntent.status },
  });
  if (rpcError) throw rpcError;

  return { credited: true, topUpId, amountUsd: Number(topUp.amount_usd || 0) };
}
