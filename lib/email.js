...
async function sendPasswordResetEmail(
  ...
  const safeFallbackUrl = escapeHtml(fallbackResetUrl || resetUrl);
  await resend.emails.send({
    from: FROM(),
    to: toEmail,
...
<div style="max-width:600px;margin:0 auto;background:#fff;border:1px solid rgba(35,47,62,.12);border-radius:22px;padding:32px;box-shadow:0 18px 48px rgba(35,47,62,.08)">
  ...
  <p style="margin:0 0 12px;color:#64748b;font-size:13px;line-height:1.5">This link expires in 30 minutes and can only be used once.</p>
  <p style="margin:0 0 12px;color:#64748b;font-size:13px;line-height:1.5">If the button opens your homepage, copy and paste this recovery link directly into your browser:<br><a href="${safeFallbackUrl}" style="color:#ff5527;word-break:break-all;text-decoration:none">${safeFallbackUrl}</a></p>
