...
function supportedCountries() {
  return (process.env.TWILIO_SUPPORTED_COUNTRIES || DEFAULT_SUPPORTED_COUNTRIES.join(","))
    .split(",")
    .map((s) => s.trim().toUpperCase())
    .filter(Boolean);
}
function lowRiskCountries() {
  return (process.env.TWILIO_LOW_RISK_VOICE_COUNTRIES || DEFAULT_LOW_RISK_COUNTRIES.join(","))
    .split(",")
    .map((s) => s.trim().toUpperCase())
    .filter(Boolean);
}
...
function normalizeNumberType(type) {
  const raw = String(type || "Local").trim().toLowerCase();
  if (raw === "tollfree" || raw === "toll_free" || raw === "toll-free") return "TollFree";
...
async function twilioRequest({ method = "GET", accountSid, path, params, fullUrl, authSid, authToken }) {
  const sid = authSid || masterSid();
  const token = authToken || masterToken();
  const targetAccountSid = accountSid || masterSid();
  if (!sid || !token) {
    throw Object.assign(new Error("TWILIO_ACCOUNT_SID and TWILIO_AUTH_TOKEN are required."), { code: "TWILIO_AUTH_MISSING" });
  }

  const baseUrl = fullUrl || `${TWILIO_BASE}/Accounts/${targetAccountSid}${path}`;
  let url = baseUrl;
  const headers = {
  ...
  if (!response.ok) {
    const err = new Error(data?.message || data?.error_message || `Twilio ${response.status}`);
...
function evaluateNumberRecommendation(number, { requiresSms = false, requiresVoice = true, country } = {}) {
  ...
  const address = String(number.addressRequirements || number.addressRequired || "none").toLowerCase();
  const capabilities = number.capabilities || {};

  if (!supported.includes(iso)) restrictionReasons.push(`Country ${iso} is not enabled for this app.`);
  if (requiresVoice && !capabilities.voice) restrictionReasons.push("Voice is not supported.");
  if (requiresSms && !capabilities.sms) restrictionReasons.push("SMS is required but not supported.");
  if (["any", "local", "foreign", "foreign_or_local"].includes(address)) restrictionReasons.push(`Address/regulatory requirement: ${address}.`);
  if (process.env.TWILIO_HIGH_RISK_COUNTRIES) {
    const highRisk = process.env.TWILIO_HIGH_RISK_COUNTRIES.split(",").map((s) => s.trim().toUpperCase()).filter(Boolean);
    if (highRisk.includes(iso)) restrictionReasons.push("Country is configured as high-risk for this app.");
  }

  let restrictionLevel = "low";
  let recommendationStatus = "recommended";
  let estimatedSetupComplexity = "easy";
  let userFacingSetupMessage = "Recommended: this number should support the selected Agently voice flow with minimal setup.";

  if (restrictionReasons.length) {
    const severe = restrictionReasons.some((r) => r.includes("not enabled") || r.includes("not supported") || r.includes("high-risk"));
  ...
  return { restrictionLevel, recommendationStatus, restrictionReasons, estimatedSetupComplexity, userFacingSetupMessage };
}

async function ensureTenantTwilioAccount({ organizationId, organizationName, createIfMissing = true }) {
  ...
  if (existing) return { ...existing, account_sid: existing.account_sid, isFallbackMaster: false };
  if (error && !isTableMissing(error) && error.code !== "PGRST116") throw error;

  if (!createIfMissing) {
    return { account_sid: masterSid(), parent_account_sid: masterSid(), isFallbackMaster: true, migrationRequired: !!error };
  }

  if (error && isTableMissing(error)) {
    throw Object.assign(new Error("Supabase migration missing: twilio_accounts table does not exist."), { code: "MIGRATION_REQUIRED" });
  }
  ...
  try {
    account = await twilioRequest({
      ...
      params: { FriendlyName: `${appName()} - ${organizationName || organizationId}`.slice(0, 64) },
    });
  } catch (err) {
    const mapped = mapTwilioError(err, "Could not create a Twilio subaccount for this tenant.");
    throw Object.assign(new Error(mapped.message), { code: mapped.code, detail: mapped.detail });
  }
  ...
  const row = {
    ...
    friendly_name: account.friendly_name || `${appName()} - ${organizationName || organizationId}`,
    status: account.status || "active",
    is_primary: true,
    raw_response: account,
  };
  const { data: saved, error: saveError } = await db.from("twilio_accounts").insert(row).select().single();
...
async function searchAvailableRecommendedNumbers(options = {}) {
  ...
  const params = { PageSize: String(Math.min(parseInt(options.limit || 40, 10), 40)) };
  ...
  return options.showAdvancedRestrictedNumbers ? mapped : mapped.filter((n) => n.recommendationStatus === "recommended");
}

async function purchaseIncomingNumber({ accountSid, phoneNumber, friendlyName, agentId, supportsSms = true }) {
  ...
  if (agentId) params.FriendlyName = `${params.FriendlyName} - ${agentId}`.slice(0, 64);
  return twilioRequest({ method: "POST", accountSid, path: "/IncomingPhoneNumbers.json", params });
}

async function fetchIncomingNumber({ accountSid, phoneSid }) {
  const data = await twilioRequest({ method: "GET", accountSid, path: `/IncomingPhoneNumbers/${phoneSid}.json` });
  return normalizeIncomingNumber(data);
}

async function listIncomingNumbers({ accountSid, pageSize = 100 } = {}) {
  const data = await twilioRequest({ method: "GET", accountSid: accountSid || masterSid(), path: "/IncomingPhoneNumbers.json", params: { PageSize: String(pageSize) } });
...
async function configureTwilioIncomingNumber(numberSid, options = {}) {
  ...
  const params = {
    ...
    VoiceFallbackUrl: options.voiceFallbackUrl || `${base}/api/twilio/voice-inbound`,
  ...
  if (supportsSms) {
    ...
    params.SmsFallbackUrl = options.smsFallbackUrl || `${base}/api/twilio/sms-inbound`;
    params.SmsFallbackMethod = options.smsFallbackMethod || "POST";
  }
  return twilioRequest({ method: "POST", accountSid: options.accountSid, path: `/IncomingPhoneNumbers/${numberSid}.json`, params });
}

async function applyVoiceDialingPermissions({ accountSid, countries = [], allowHighRiskSpecialNumbers = false, allowHighRiskTollFraudNumbers = false }) {
  const normalized = [...new Set((countries || []).map(normalizeCountry).filter(Boolean))];
  ...
  const updates = normalized.map((isoCountry) => ({
    ...
    blocked: !lowRisk.includes(isoCountry) && !allowHighRiskSpecialNumbers && !allowHighRiskTollFraudNumbers,
  }));
  const safeUpdates = updates.filter((u) => !u.blocked);
  if (!safeUpdates.length) {
    return { success: false, results: updates, message: "No countries were enabled because all requested countries are outside the configured low-risk list." };
  }
  const data = await twilioRequest({
    method: "POST",
    accountSid,
...
function buildManualSmsGeoInstructions(countries = []) {
  const list = countries.map(normalizeCountry).join(", ") || "the selected destination countries";
  return [
