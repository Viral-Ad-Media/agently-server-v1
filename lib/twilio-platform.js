...
function normalizeCountry(country) {
  const value = normalizeEnvCountryToken(country);
  if (value === "UK") return "GB";
  return value || "US";
}

function normalizeCountryList(countries = []) {
  const input = Array.isArray(countries) ? countries : [countries];
  return [...new Set(input.map(normalizeCountry).filter(Boolean))];
}
...
function evaluateNumberRecommendation(
  ...
  const sellableCountries = supported.filter((iso) => lowRisk.includes(iso));
  ...
  if (!sellableCountries.includes(iso))
    restrictionReasons.push(
      `Country ${iso} is not enabled for low-restriction tenant purchases.`,
    );
  if (number.beta === true)
    restrictionReasons.push(
      "Beta phone-number inventory is not offered for tenant purchases.",
    );
  ...
  // PATCH: address requirements are a REGULATORY FOLLOW-UP, not a block.
  // GB/AU/IE/NZ local numbers ALWAYS report "any" or "local". Treating that as
  // a restriction is what filtered every non-US/CA number out of search.
  if (["any", "local", "foreign", "foreign_or_local"].includes(address))
    regulatoryFollowUps.push(
      `Local regulations in ${iso} require a registered address for this number. ` +
        `Calls work immediately; complete verification from Settings.`,
    );
  ...
  return {
    ...
    restrictionReasons,
    regulatoryFollowUps,
    requiresRegulatoryFollowUp: regulatoryFollowUps.length > 0,
...
async function searchAvailableRecommendedNumbers(options = {}) {
  ...
  // PATCH: AreaCode is a North-America-only Twilio parameter. Sending it for
  // GB/AU/IE/NZ returns an empty set, which read to tenants as "the area code
  // filter stopped working". Outside NANP we downgrade it to a number prefix.
  const NANP = ["US", "CA"];
  const cleanArea = String(options.areaCode || "").replace(/\D/g, "");
  const cleanContains = String(options.contains || "").replace(/\D/g, "");
  if (cleanArea && NANP.includes(country)) {
    params.AreaCode = cleanArea;
  } else if (cleanArea) {
    params.Contains = cleanContains
      ? `${cleanArea}${cleanContains}`
      : cleanArea;
  }
  if (cleanContains && !params.Contains) params.Contains = cleanContains;
...
async function releaseIncomingNumber({ accountSid, phoneSid }) {
  if (!phoneSid) {
    throw Object.assign(new Error("phoneSid is required."), {
      code: "PHONE_SID_REQUIRED",
    });
  }
...
async function applyVoiceDialingPermissions({
  ...
  const lowRisk = normalizeCountryList(lowRiskCountries());
  const lowRiskSet = new Set(lowRisk);
  const updates = normalized.map((isoCountry) => ({
    ...
    blocked:
      !lowRiskSet.has(isoCountry) &&
      !allowHighRiskSpecialNumbers &&
      !allowHighRiskTollFraudNumbers,
  }));
  const safeUpdates = updates.filter((u) => !u.blocked);
  const countriesAllowedAfterLowRiskFilter = safeUpdates.map((u) => u.iso_code);
  const debug = {
    lowRiskCountriesConfigured: lowRisk,
    requestedCountriesNormalized: normalized,
  ...
  return { success: true, update: data, results: updates, ...debug };
}
...
/**
 * PATCH — THIS FUNCTION WAS IMPORTED BY api/routes/twilio.js:90 BUT NEVER
 * DEFINED OR EXPORTED. Calling it threw `TypeError: ... is not a function`
 * AFTER the number had already been bought and configured, which the outer
 * catch flattened into "Could not purchase and configure this Twilio number."
 * Every failure since then orphaned a paid number. This is that fix.
 *
 * Deliberately more permissive than evaluateNumberRecommendation(): by this
 * point money is spent, so we only reject where the number genuinely cannot
 * serve voice. Softer conditions become follow-ups and the number is kept.
 */
function isRecommendedForAutomaticPurchase(number, options = {}) {
  const {
    requiresVoice = true,
    requiresSms = false,
  ...
  const iso = normalizeCountry(
    number?.isoCountry || number?.country || expectedCountry,
  );
  const capabilities = number?.capabilities || {};
  const address = String(number?.addressRequirements || "none").toLowerCase();

  const blockingReasons = [];
  const warnings = [];

  if (requiresVoice && capabilities.voice === false)
    blockingReasons.push("This number cannot receive or place voice calls.");
  if (requiresSms && capabilities.sms === false)
    blockingReasons.push("SMS was required but this number cannot send SMS.");
  if (!supportedCountries().includes(iso))
    blockingReasons.push(`Numbers in ${iso} are not enabled on this platform.`);

  if (process.env.TWILIO_HIGH_RISK_COUNTRIES) {
    const highRisk = String(process.env.TWILIO_HIGH_RISK_COUNTRIES)
      .split(",")
      .map((s) => s.trim().toUpperCase())
      .filter(Boolean);
    if (highRisk.includes(iso))
      blockingReasons.push(`${iso} is blocked for automatic activation.`);
  }

  if (["any", "local", "foreign", "foreign_or_local"].includes(address)) {
    warnings.push(
      `Local regulations in ${iso} require a registered address on file. ` +
        `Calls work now; complete verification to avoid future carrier action.`,
    );
  }

  const base = apiBaseUrl();
  if (base && number?.voiceUrl && !String(number.voiceUrl).startsWith(base))
    warnings.push("Voice webhook was not pointing at Agently; it was reset.");

  const safe = blockingReasons.length === 0;

  return {
    safeForAutomaticPurchase: safe,
    safe,
    recommended: safe,
    isoCountry: iso,
    blockingReasons,
    restrictionReasons: blockingReasons,
    warnings,
    requiresRegulatoryFollowUp: warnings.length > 0,
    capabilities: {
      voice: capabilities.voice !== false,
      sms: capabilities.sms === true,
      mms: capabilities.mms === true,
    },
    userFacingSetupMessage: safe
      ? warnings.length
        ? "Number is live for calls. One optional verification step remains."
        : "Number is live and ready for inbound and outbound calls."
      : blockingReasons[0],
  };
}

/** Pre-purchase gate, same rules, so search and purchase can never disagree. */
function isSellableInSearch(number, options = {}) {
  const verdict = isRecommendedForAutomaticPurchase(number, options);
  return verdict.safeForAutomaticPurchase && number?.beta !== true;
}

/** Countries we can actually sell in: supported INTERSECT low-risk. */
function sellableCountries() {
  const supported = supportedCountries();
  const lowRisk = lowRiskCountries();
  return supported.filter((iso) => lowRisk.includes(iso));
}
