...
async function generateStreamingResponse(messages, onToken, usageContext = {}) {
  ...
  try {
    ...
    await logOpenAIUsage({
      ...
      eventType: usageContext.eventType || "openai_streaming_estimated_tokens",
      model: usageContext.model || "gpt-4o-mini",
      ...
      knowledgeBaseId:
        usageContext.knowledgeBaseId || usageContext.knowledge_base_id || null,
      metadata: {
        estimated: true,
        method: "chars_div_4",
    ...
    console.warn(
      "[usage-ledger] OpenAI streaming usage estimate skipped",
      err.message || String(err),
    );
  }
}
