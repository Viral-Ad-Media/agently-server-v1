...
async function generateFaqsFromKnowledgeContent({
  website = "",
  chunks = [],
  scrapeReport = null,
} = {}) {
  const openai = getOpenAI();
  const compactChunks = (chunks || [])
    .map((chunk, index) => {
      const title =
        chunk.source_title || chunk.sourceTitle || `Page ${index + 1}`;
      const url = chunk.source_url || chunk.sourceUrl || website || "";
      const content = String(
        chunk.compact_summary || chunk.content || "",
      ).slice(0, 1200);
      return `Source: ${title}\nURL: ${url}\nContent: ${content}`;
    })
    .filter(Boolean)
  ...
  const reportLine = scrapeReport
    ? `Scrape coverage: ${scrapeReport.pagesScraped || 0}/${scrapeReport.pagesAttempted || 0} pages scraped, ${scrapeReport.chunksStored || chunks.length || 0} knowledge chunks stored, ${scrapeReport.productsFound || 0} products found.`
    : "";

  const prompt = `You are helping set up Agently, an AI agent platform for calls, chat, follow-ups, and lead capture.
Based only on the scraped website knowledge below, generate exactly 8 useful FAQ entries a customer might ask.
Use concrete details from the content when available. Avoid inventing prices, addresses, or policies that are not present.
Return ONLY valid JSON: {"faqs":[{"question":"...","answer":"..."}]}
No markdown. No explanation. Just the JSON object.
...
Scraped knowledge:
${compactChunks || `Business website: ${website}`}`;
