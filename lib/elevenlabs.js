...
const DEFAULT_PREVIEW_TEXT = "Hello! This is a voice preview from Agently!";
...
function serializeElevenLabsVoice(voice) {
  ...
  const name = voice.name || voice.display_name || voiceId;
  const previewUrl = voice.preview_url || voice.previewUrl || null;
...
async function fetchElevenLabsVoices() {
  ...
  // ElevenLabs /v1/voices returns every voice available to the API key in data.voices.
  // Do not cap or replace this with a short built-in test list.
  return (data?.voices || [])
    .map(serializeElevenLabsVoice)
    .filter((v) => v.voiceId || v.voice_id)
    .sort((a, b) =>
...
async function listVoiceCatalog({
  db,
  provider = "elevenlabs",
  source = "api",
  preferApi = true,
} = {}) {
  const requestedSource = String(source || "api").toLowerCase();

  // Production default: fetch the full ElevenLabs account voice list from the API.
  // The local voice_catalog table is only a fallback/cache, because it may contain
  // a short seed list (for example 3 voices) and must not restrict the frontend dropdown.
  if (
    provider === "elevenlabs" &&
    ...
    try {
      ...
      return { source: "elevenlabs_api", voices, count: voices.length };
    } catch (error) {
      ...
      } else if (requestedSource === "api") {
        throw error;
      }
      console.warn(
        "[voices] ElevenLabs API voice fetch failed, falling back to catalog:",
  ...
  if (db) {
    try {
      ...
      if (Array.isArray(data) && data.length) {
        const voices = data.map(serializeCatalogRow);
        return {
          source: "database",
          voices,
          count: voices.length,
          warning:
            provider === "elevenlabs"
              ? "Using cached voice_catalog fallback. Check ELEVENLABS_API_KEY if this list is incomplete."
              : undefined,
        };
      }
    } catch (error) {
  ...
  return { source: "empty", voices: [], count: 0 };
}
