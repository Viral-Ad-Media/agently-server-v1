...
async function listVoices(options = {}) {
  const result = await listVoiceCatalog(options);
...
async function getVoiceSettings(voiceId) {
  ...
  const res = await fetch(
    `${ELEVENLABS_API_BASE}/voices/${encodeURIComponent(id)}/settings`,
    {
      method: "GET",
  ...
  if (!res.ok) {
    const err = new Error(
      data?.detail?.message ||
        data?.message ||
        `ElevenLabs settings request failed: ${res.status}`,
    );
    err.status = res.status;
    err.raw = data;
    throw err;
...
async function synthesizeSpeech({
  ...
  return synthesizeElevenLabsPreview({
    voiceId,
    text,
    modelId,
    outputFormat,
...
async function streamSpeechIfSupported({
  text,
  voiceId,
  voiceSettings = {},
  modelId,
  ...
  if (typeof onAudioChunk === "function" && audio.buffer?.length) {
    await onAudioChunk(audio.buffer, { bytes: audio.buffer.length, chunks: 1 });
  }
  return audio;
}
