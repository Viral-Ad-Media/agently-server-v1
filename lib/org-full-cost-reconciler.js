...
const PROVIDER_RECONCILIATION_STEPS = Object.freeze([
  "mappings",
  "twilio",
  "openai",
  "elevenlabs",
...
function normalizeProviderSteps(providers) {
  const aliases = new Map([
    ["mapping", "mappings"],
    ["provider_mapping", "mappings"],
    ["provider_resource_mapping", "mappings"],
    ["all", "all"],
    ["internal", "agently_internal"],
    ["agently", "agently_internal"],
    ["chatbot", "agently_internal"],
    ["knowledge", "agently_internal"],
    ["kb", "agently_internal"],
  ...
  const normalized = raw.map(
    (item) =>
      aliases.get(String(item || "").toLowerCase()) ||
      String(item || "").toLowerCase(),
  );
  if (!normalized.length || normalized.includes("all")) {
    return [...PROVIDER_RECONCILIATION_STEPS];
  }
  const allowed = new Set(PROVIDER_RECONCILIATION_STEPS);
  const unique = [];
  for (const item of normalized) {
    if (!allowed.has(item)) {
      throw new Error(
        `Unsupported reconciliation provider '${item}'. Allowed values: ${PROVIDER_RECONCILIATION_STEPS.join(", ")}, all.`,
      );
    }
...
function shouldRunProvider(selectedProviders, provider) {
...
async function syncProviderMappingsFromUsageEvents({ organizationId }) {
  ...
  for (const row of rows) {
    ...
    if (provider === "twilio") {
    ...
    } else if (provider === "aws" || provider === "railway") {
      addRuntimeMappingCandidate(
        candidates,
        provider,
        "runtime_session",
      ...
      if (provider === "railway") {
        // Historical rows only. Nothing writes Railway data any more, and
        // service/project ids are Railway concepts with no AWS equivalent —
        // so they are read for old rows and never produced for new ones.
        addRuntimeMappingCandidate(
          candidates,
          "railway",
          "service_id",
...
async function reconcileRuntimeAllocation({
  organizationId,
  start,
  end,
  callRecords,
...
async function runOrgFullCostReconciliation({
  ...
  const selectedProviders = normalizeProviderSteps(providers);
  const sb = getSupabase();
  ...
  if (shouldRunProvider(selectedProviders, "mappings")) {
    steps.push(
    ...
    steps.push({
      ...
      ok: true,
      skipped: true,
      reason: "provider step not selected",
    });
  }
  providerResources = await loadProviderResourcesForOrg(organizationId);
  ...
  if (shouldRunProvider(selectedProviders, "twilio")) {
    try {
      steps.push(
  ...
  if (shouldRunProvider(selectedProviders, "openai")) {
    try {
      steps.push(
  ...
  if (shouldRunProvider(selectedProviders, "elevenlabs")) {
    try {
      steps.push(
  ...
  if (shouldRunProvider(selectedProviders, "aws")) {
    try {
      steps.push(
        await reconcileRuntimeAllocation({
          organizationId,
          start: effectiveStart,
          end: effectiveEnd,
          callRecords: callRecordsResult.rows,
  ...
  if (shouldRunProvider(selectedProviders, "supabase")) {
    try {
  ...
  if (shouldRunProvider(selectedProviders, "agently_internal")) {
    try {
      steps.push(
