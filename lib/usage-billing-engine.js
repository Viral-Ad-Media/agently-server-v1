 ...
 *
 * They come from billing_platform_settings when set, because Lightsail has no
 * "update one env var" call — editing an env var there redeploys the live call
 * path, which is far too blunt an instrument for correcting a price. `override`
 * is that settings row; the environment is the fallback.
 */
function sharedMonthlyCostUsd(env = process.env, override = null) {
  const pick = (fromSettings, fromEnv) =>
    fromSettings === null || fromSettings === undefined
      ? safeNumber(fromEnv, 0)
      : safeNumber(fromSettings, 0);
  const pools = {
    lightsail: pick(override?.lightsail, env.AWS_LIGHTSAIL_MONTHLY_USD),
    supabase: pick(override?.supabase, env.SUPABASE_MONTHLY_USD),
    other: pick(override?.other, env.SHARED_INFRA_MONTHLY_USD),
  };
...
function sharedHourlyCostUsd(env = process.env, override = null) {
  const monthly = sharedMonthlyCostUsd(env, override);
  return {
    ...monthly,
...
async function runBillingCycle(db, options = {}) {
  ...
  const shared = sharedMonthlyCostUsd(env, settings.infraCosts);
  ...
  if (!shared.configured) {
    summary.warnings.push(
      "No shared infrastructure cost configured (billing_platform_settings.aws_lightsail_monthly_usd / supabase_monthly_usd are null and the env vars are unset), so nothing is apportioned. Set them from the real invoices.",
    );
  }
