...
async function resolveAssignedKnowledgeBaseId(db, orgId, agentRow) {
  ...
  const linkId = cleanId(links?.[0]?.knowledge_base_id);
  if (direct && linkId && direct !== linkId) {
    console.warn("[context-builder] KB assignment mismatch; direct agent value wins", {
      orgId,
      agentId: agentRow.id,
      direct,
      linkId,
      directUpdatedAt: agentRow?.updated_at || null,
      linkCreatedAt: links?.[0]?.created_at || null,
    });
  }
  return direct || linkId;
}
