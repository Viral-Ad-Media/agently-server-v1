...
async function resolveAssignedKnowledgeBaseId(db, orgId, agentRow) {
  ...
  return cleanId(links?.[0]?.knowledge_base_id);
}
...
async function loadVoiceContext(db, orgId, agentRow, query, extra = {}) {
  ...
  if (knowledgeBaseId) {
    ...
    const [faqRows, linkedChatbotRows, directChunkRows] = await Promise.all([
      safeQuery(
        "faqs by voice_agent_id",
        () =>
          db
            .from("faqs")
            .select("*")
    ...
    const linkedChatbotIds = (linkedChatbotRows || [])
      .map((row) => row.id)
      .filter(Boolean);
    let linkedChunks = [];
    if (linkedChatbotIds.length > 0) {
      linkedChunks = await safeQuery(
        "linked chatbot chunks",
        () =>
          db
            .from("knowledge_chunks")
            .select("*")
    ...
    faqs = normalizeFaqs(faqRows || []);
    allChunks = [...(directChunkRows || []), ...(linkedChunks || [])];
  }
