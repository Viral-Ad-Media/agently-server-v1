...
async function loadKnowledgeBasesByIds(
  ...
  try {
    const { data, error } = await db
      ...
      .select("id,name,business_name,industry,primary_url,domain")
...
function knowledgeIdentityName(context) {
  ...
  return cleanIdentity(
    base?.business_name ||
      base?.name ||
      base?.domain ||
      context?.chatbot?.header_title ||
      context?.chatbot?.name ||
      context?.agentRow?.name ||
      "this knowledge base",
  );
}
...
function knowledgeIdentityWebsite(context) {
  const base = primaryKnowledgeBase(context);
  return cleanIdentity(base?.primary_url || base?.domain || "");
}
...
function normalizeChunk(row) {
  return {
    id: row?.id || null,
    content: String(row?.compact_summary || row?.content || "").trim(),
...
function buildChatPrompt({
  ...
  const parts = [];
  parts.push(
    `You are the website assistant for ${businessName || "this knowledge base"}${website ? ` at ${website}` : ""}.`,
  );
  parts.push(
...
async function loadChatbotContext(db, chatbotId, query) {
  const { data: chatbot } = await db
    .from("chatbots")
    .select(
      "id, organization_id, voice_agent_id, knowledge_base_id, header_title, custom_prompt, faqs, collect_leads",
    )
    .eq("id", chatbotId)
    .single();
  ...
  // Strict isolation: legacy chatbot.faqs are not scoped by knowledge_base_id,
  // so ignore them after a Knowledge Base is assigned.
  const faqJson = knowledgeBaseIds.length
    ? []
    : normalizeFaqs(Array.isArray(chatbot.faqs) ? chatbot.faqs : []);
  let scopedFaqs = [];
  let rawChunks = [];

  if (knowledgeBaseIds.length) {
    scopedFaqs = await searchScopedFaqs(db, {
      organizationId: chatbot.organization_id,
    ...
    rawChunks = await searchScopedKnowledgeChunks(db, {
      organizationId: chatbot.organization_id,
      chatbotId: chatbot.id,
      knowledgeBaseIds,
      query,
    ...
    const chunkQueries = [
      db
        ...
        .select(
          "id, content, compact_summary, source_url, source_title, chatbot_id, voice_agent_id, metadata",
        )
  ...
  return {
    ...
    systemPrompt: buildChatPrompt({
      businessName: knowledgeIdentityName({ chatbot, knowledgeBases }),
      website: knowledgeIdentityWebsite({ knowledgeBases }),
...
async function loadVoiceContext(db, orgId, agentRow, query, extra = {}) {
  ...
  const linkedChatbotResult = await db
    .from("chatbots")
    .select("id")
    .eq("organization_id", orgId)
    .eq("voice_agent_id", agentRow.id)
    .limit(5);
  const linkedChatbotIds = (linkedChatbotResult.data || [])
    .map((row) => row.id)
    .filter(Boolean);
  ...
  if (knowledgeBaseIds.length) {
    ...
    allChunks = await searchScopedKnowledgeChunks(db, {
      organizationId: orgId,
      voiceAgentId: agentRow.id,
      linkedChatbotIds,
      knowledgeBaseIds,
    ...
    const [faqResult, directChunksResult] = await Promise.all([
      db
        .from("faqs")
    ...
    faqs = normalizeFaqs(faqResult.data || []);
    allChunks = [...(directChunksResult.data || []), ...linkedChunks];
  }
