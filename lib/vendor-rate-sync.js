...
function normalizeProvider(value) {
  ...
  if (["knowledge-base", "knowledgebase", "kb", "knowledge"].includes(p))
    return "knowledge_base";
  if (["internal", "agently_internal", "agently-internal"].includes(p))
    return "agently";
  return p;
}
...
function normalizeRateInput(row, source = "env") {
  ...
  const eventType =
    safeString(row.eventType || row.event_type || row.type || "*") || "*";
  ...
  if (
    ...
    !unit ||
    unitCostUsd == null ||
    Number.isNaN(Number(unitCostUsd))
  )
    return null;
  return {
    provider,
    ...
    effectiveFrom:
      safeString(row.effectiveFrom || row.effective_from) || nowIso(),
...
function envRateCardsForProvider(provider) {
  ...
  for (const entry of Array.isArray(general) ? general : []) {
    ...
    if (normalizedRow && normalizedRow.provider === normalized)
      rows.push(normalizedRow);
  }
  for (const entry of Array.isArray(providerSpecific) ? providerSpecific : []) {
    const normalizedRow = normalizeRateInput(
      { provider: normalized, ...entry },
      providerEnvName,
    );
  ...
  const scalarMap = {
    twilio: [
      ...
      [
        "TWILIO_RECORDING_COST_PER_MINUTE_USD",
        "recordings",
        "recording_minutes",
        "minutes",
      ],
      [
        "TWILIO_NUMBER_PURCHASE_COST_USD",
        "phone_number",
        "number_purchase",
        "number",
      ],
      [
        "TWILIO_NUMBER_MONTHLY_RENTAL_USD",
        "phone_number",
        "monthly_rental",
        "number_month",
      ],
    ...
    openai: [
      [
        "OPENAI_REALTIME_INPUT_TOKEN_COST_USD",
        "realtime",
        "input_tokens",
        "tokens",
      ],
      [
        "OPENAI_REALTIME_OUTPUT_TOKEN_COST_USD",
        "realtime",
        "output_tokens",
        "tokens",
      ],
      [
        "OPENAI_REALTIME_AUDIO_INPUT_TOKEN_COST_USD",
        "realtime",
        "audio_input_tokens",
        "tokens",
      ],
      [
        "OPENAI_REALTIME_AUDIO_OUTPUT_TOKEN_COST_USD",
        "realtime",
        "audio_output_tokens",
        "tokens",
      ],
    ...
    railway: [
      ["RAILWAY_COST_PER_RUNTIME_SECOND_USD", "runtime", "*", "seconds"],
    ],
    supabase: [
      ["SUPABASE_STORAGE_COST_PER_BYTE_MONTH_USD", "storage", "*", "bytes"],
      [
        "SUPABASE_COMPUTE_COST_PER_ORG_DAY_USD",
        "database_compute",
        "daily_compute_allocation",
        "org_day",
      ],
    ],
    resend: [
      ["RESEND_COST_PER_EMAIL_USD", "transactional_email", "*", "email"],
    ],
  };
  ...
  const gbMonth = safeString(
    process.env.SUPABASE_STORAGE_COST_PER_GB_MONTH_USD,
  );
  if (normalized === "supabase" && gbMonth && !Number.isNaN(Number(gbMonth))) {
    rows.push({
      ...
      effectiveFrom: nowIso(),
      notes:
        "Converted from SUPABASE_STORAGE_COST_PER_GB_MONTH_USD to byte-month equivalent for storage snapshots.",
      source: "SUPABASE_STORAGE_COST_PER_GB_MONTH_USD",
      metadata: {
        env_name: "SUPABASE_STORAGE_COST_PER_GB_MONTH_USD",
        source: "converted_env",
        gb_month_usd: safeNumber(gbMonth),
      },
    });
  }
...
function rateChanged(existing, next) {
  ...
  return (
    Math.abs(
      Number(existing.unit_cost_usd || 0) - Number(next.unitCostUsd || 0),
    ) > 0.0000000001
  );
}
...
async function upsertEffectiveRateCard(rate) {
  const normalized = normalizeRateInput(
    rate,
    rate?.source || "vendor_rate_sync",
  );
  if (!normalized)
    return { ok: false, skipped: true, reason: "invalid_rate_card", rate };
  const sb = getSupabase();
  ...
  const { data: activeRows, error: activeErr } = await sb
    ...
    .select(
      "id,unit_cost_usd,effective_from,effective_to,metadata,source,notes",
    )
    .match(exact)
  ...
  if (activeRows?.length) {
    ...
    if (ids.length) {
      await sb
        .from("billing_rate_cards")
        .update({ effective_to: when, updated_at: nowIso() })
        .in("id", ids);
    }
  }
...
async function fetchJson(url, options = {}) {
  ...
  if (!res.ok) {
    const msg =
      body?.message || body?.error?.message || text || `HTTP ${res.status}`;
...
function normalizeTwilioNumberType(value) {
  return (
    safeString(value || "phone_number")
      .toLowerCase()
      .replace(/[^a-z0-9]+/g, "_")
      .replace(/^_+|_+$/g, "") || "phone_number"
  );
}
...
function collectTwilioPhoneNumberRates(json, country) {
  const rates = [];
  const prices =
    json?.phone_number_prices || json?.phoneNumberPrices || json?.prices || [];
  for (const item of Array.isArray(prices) ? prices : []) {
    const type = normalizeTwilioNumberType(
      item.number_type || item.numberType || item.type || item.category,
    );
    const monthly =
      item.current_price ??
      item.currentPrice ??
    ...
    const monthlyCost = Math.abs(safeNumber(monthly));
    const countryCode = safeString(country).toLowerCase();

    // Keep the country/type-specific monthly rate for audit and future exact matching.
    // The generic number_purchase and daily_prorated_number_rental fallbacks are
    // derived after all countries are fetched so the fallback rate is conservative.
    rates.push({
      provider: "twilio",
      service: "phone_number",
      eventType: `monthly_rental_${countryCode}_${type}`,
      unit: "number_month",
      unitCostUsd: monthlyCost,
      source: "twilio_pricing_api_v1_phone_numbers",
      notes: `Twilio ${country} ${type} monthly phone-number rental price from Pricing API v1.`,
      metadata: { country, number_type: type, raw_item: item },
    });
  }
  return rates;
}

function deriveTwilioPhoneNumberFallbackRates(phoneNumberRates = []) {
  const monthlyRates = (Array.isArray(phoneNumberRates) ? phoneNumberRates : [])
    .filter(
      (rate) =>
        rate &&
        rate.provider === "twilio" &&
        rate.service === "phone_number" &&
        rate.unit === "number_month",
    )
    .map((rate) => ({ rate, cost: safeNumber(rate.unitCostUsd) }))
    .filter((row) => Number.isFinite(row.cost) && row.cost >= 0);
  if (!monthlyRates.length) return [];

  // The current billing_rate_cards schema does not have country/number_type columns.
  // Use the highest synced monthly price as the generic fallback so the platform
  // does not understate production cost when exact per-number metadata is absent.
  const max = monthlyRates.reduce(
    (best, row) => (row.cost > best.cost ? row : best),
    monthlyRates[0],
  );
  const maxMonthly = max.cost;
  const sourceRates = monthlyRates.map(({ rate, cost }) => ({
    eventType: rate.eventType,
    unitCostUsd: cost,
    country: rate.metadata?.country || null,
    numberType: rate.metadata?.number_type || null,
  }));
  ...
  return [
    {
      ...
      unit: "number_month",
      unitCostUsd: maxMonthly,
      source: "twilio_pricing_api_v1_phone_numbers_conservative_fallback",
      notes:
        "Conservative generic Twilio monthly phone-number rental fallback using the highest synced country/type price.",
      metadata: { selected_rate: max.rate, source_rates: sourceRates },
    },
    {
      provider: "twilio",
      service: "phone_number",
      eventType: "daily_prorated_number_rental",
      unit: "number_day",
      unitCostUsd: maxMonthly / AVERAGE_DAYS_PER_MONTH,
      source: "twilio_pricing_api_v1_phone_numbers_conservative_fallback",
      notes:
        "Conservative generic Twilio daily prorated phone-number rental fallback derived from monthly Pricing API v1 data.",
      metadata: {
        selected_monthly_rate: max.rate,
        days_per_month: AVERAGE_DAYS_PER_MONTH,
    ...
    },
    {
      ...
      source: "twilio_pricing_api_v1_phone_numbers_conservative_fallback",
      notes:
        "Conservative generic Twilio number purchase/first-month cost fallback using the highest synced monthly phone-number price.",
      metadata: { selected_monthly_rate: max.rate, source_rates: sourceRates },
    },
  ];
}
...
function collectTwilioVoiceRates(json, country) {
  ...
  const arrays = [
    [
      "outbound_call",
      json?.outbound_prefix_prices || json?.outboundPrefixPrices || [],
    ],
    [
      "inbound_call",
      json?.inbound_call_prices || json?.inboundCallPrices || [],
    ],
  ];
  for (const [eventType, arr] of arrays) {
    for (const item of Array.isArray(arr) ? arr : []) {
      const price =
        item.current_price ?? item.base_price ?? item.price ?? item.price_usd;
...
async function syncTwilioRates({
  startAt,
  endAt,
  organizationId,
  dryRun = false,
  ...
  const accountSid = safeString(
    process.env.TWILIO_ACCOUNT_SID || process.env.TWILIO_PARENT_ACCOUNT_SID,
  );
  ...
  if (accountSid && authToken) {
    for (const country of countries) {
      try {
        const voice = await fetchJson(
          `https://pricing.twilio.com/v2/Voice/Countries/${encodeURIComponent(country)}`,
          {
        ...
        snapshots.push(
          await insertSnapshot({
            provider: "twilio",
            service: "voice",
            snapshotType: "twilio_voice_pricing_api",
            organizationId,
            periodStart: startAt,
            periodEnd: endAt,
            rawPayload: voice,
            normalizedRates: normalized,
            metadata: { country },
          }),
        );
      } catch (err) {
        warnings.push(
          `Twilio voice pricing ${country} skipped: ${err.message || String(err)}`,
        );
      }
      try {
        const numbers = await fetchJson(
          `https://pricing.twilio.com/v1/PhoneNumbers/Countries/${encodeURIComponent(country)}`,
          {
            headers: { Authorization: basicAuth(accountSid, authToken) },
          },
        );
        ...
        snapshots.push(
          await insertSnapshot({
            provider: "twilio",
            service: "phone_number",
            snapshotType: "twilio_phone_number_pricing_api_v1",
            organizationId,
            periodStart: startAt,
            periodEnd: endAt,
            rawPayload: numbers,
        ...
        warnings.push(
          `Twilio phone-number pricing ${country} skipped: ${err.message || String(err)}`,
        );
      }
    }
  } else {
    warnings.push(
      "TWILIO_ACCOUNT_SID/TWILIO_AUTH_TOKEN not set; using env JSON/scalar rate cards only.",
    );
  }

  const phoneFallbackRates = deriveTwilioPhoneNumberFallbackRates(rates);
  rates = rates.concat(phoneFallbackRates);
  ...
  const upsert = dryRun
    ...
    : await upsertRateCards(rates);
  return {
    provider: "twilio",
    ratesDiscovered: rates.length,
    upsert,
    warnings,
...
async function syncOpenAIRates({
  startAt,
  endAt,
  organizationId,
  dryRun = false,
    ...
    try {
      ...
      const data = await fetchJson(url.toString(), {
        headers: {
          ...
          "OpenAI-Organization":
            safeString(process.env.OPENAI_ORG_ID) || undefined,
        },
      });
      await insertSnapshot({
        provider: "openai",
        service: "all",
        snapshotType: "openai_cost_api",
        organizationId,
        periodStart: startAt,
        periodEnd: endAt,
        rawPayload: data,
        normalizedRates: rates,
        metadata: { api: "organization_costs" },
      });
      snapshots += 1;
    } catch (err) {
      warnings.push(
        `OpenAI cost API snapshot skipped: ${err.message || String(err)}`,
      );
    }
  } else {
    warnings.push(
      "OPENAI_ADMIN_KEY/OPENAI_API_KEY not set; using env rate cards only.",
    );
  }
  const upsert = dryRun
    ? {
...
async function syncElevenLabsRates({
  startAt,
  endAt,
  organizationId,
  dryRun = false,
  ...
  if (key) {
    try {
      ...
      const overage =
        subscription?.overage_price_per_character ??
        subscription?.overagePricePerCharacter;
      if (overage != null && !Number.isNaN(Number(overage))) {
        rates.push({
          ...
          eventType: "*",
          unit: "characters",
          unitCostUsd: safeNumber(overage),
          source: "elevenlabs_subscription_api",
          notes:
            "ElevenLabs overage price per character from subscription API.",
          metadata: { subscription_tier: subscription?.tier || null },
        });
      }
      await insertSnapshot({
        provider: "elevenlabs",
        service: "subscription",
        snapshotType: "elevenlabs_subscription_api",
        organizationId,
        periodStart: startAt,
        periodEnd: endAt,
      ...
      warnings.push(
        `ElevenLabs subscription snapshot skipped: ${err.message || String(err)}`,
      );
    }
    try {
      ...
      usageUrl.searchParams.set(
        "start_unix",
        String(timestampSeconds(startAt)),
      );
      ...
      await insertSnapshot({
        provider: "elevenlabs",
        service: "usage",
        snapshotType: "elevenlabs_usage_api",
        organizationId,
        periodStart: startAt,
        periodEnd: endAt,
        rawPayload: usage,
      ...
      warnings.push(
        `ElevenLabs usage snapshot skipped: ${err.message || String(err)}`,
      );
    }
  } else {
...
async function syncRailwayRates({
  startAt,
  endAt,
  organizationId,
  dryRun = false,
  ...
  if (key && projectId) {
    try {
      ...
      await insertSnapshot({
        provider: "railway",
        service: "project",
        snapshotType: "railway_graphql_project_snapshot",
        organizationId,
        periodStart: startAt,
        periodEnd: endAt,
        rawPayload: data,
        normalizedRates: rates,
        metadata: { project_id: projectId, service_id: serviceId || null },
      });
      snapshots += 1;
    } catch (err) {
      warnings.push(
        `Railway GraphQL snapshot skipped: ${err.message || String(err)}`,
      );
    }
  } else {
    warnings.push(
      "RAILWAY_API_TOKEN/RAILWAY_PROJECT_ID not set; using env rate cards only.",
    );
  }
  const upsert = dryRun
    ? {
...
async function syncSupabaseRates({
  startAt,
  endAt,
  organizationId,
  dryRun = false,
  ...
  if (token && projectRef) {
    try {
      ...
      await insertSnapshot({
        provider: "supabase",
        service: "project",
        snapshotType: "supabase_management_project_snapshot",
        organizationId,
        periodStart: startAt,
        periodEnd: endAt,
        rawPayload: data,
      ...
      warnings.push(
        `Supabase project snapshot skipped: ${err.message || String(err)}`,
      );
    }
  } else {
    warnings.push(
      "SUPABASE_ACCESS_TOKEN/SUPABASE_PROJECT_REF not set; using env rate cards only.",
    );
  }
  const upsert = dryRun
    ? {
...
async function syncResendRates({
  startAt,
  endAt,
  organizationId,
  dryRun = false,
  ...
  if (key) {
    try {
      ...
      const data = await fetchJson("https://api.resend.com/domains", {
        headers: { Authorization: `Bearer ${key}` },
      });
      await insertSnapshot({
        provider: "resend",
        service: "account",
        snapshotType: "resend_api_reachability_snapshot",
        organizationId,
        periodStart: startAt,
        periodEnd: endAt,
        rawPayload: data,
      ...
      warnings.push(
        `Resend API snapshot skipped: ${err.message || String(err)}`,
      );
    }
  } else {
...
async function syncInternalEnvRates({
  provider,
  organizationId,
  startAt,
  endAt,
  ...
  const rates = envRateCardsForProvider(normalizedProvider);
  const warnings = [];
  if (!rates.length) {
    warnings.push(
      `${normalizedProvider} has no matching VENDOR_RATE_CARD_JSON entries. Add entries with provider=\"${normalizedProvider}\".`,
    );
  }
  if (!dryRun && rates.length) {
    await insertSnapshot({
      provider: normalizedProvider,
      service: "internal_rate_card",
      snapshotType: "env_rate_card_snapshot",
      organizationId,
      periodStart: startAt,
      periodEnd: endAt,
      rawPayload: rates,
      normalizedRates: rates,
      metadata: { source: "VENDOR_RATE_CARD_JSON", internal_provider: true },
    });
  }
  const upsert = dryRun
    ? {
...
const PROVIDER_SYNCERS = {
  ...
  agently: (args) => syncInternalEnvRates({ ...args, provider: "agently" }),
  knowledge_base: (args) =>
    syncInternalEnvRates({ ...args, provider: "knowledge_base" }),
};
...
async function runVendorRateSync({
  ...
  for (const provider of normalizedProviders.length
    ? normalizedProviders
    : DEFAULT_PROVIDERS) {
    ...
    const runId = await createSyncRun({
      ...
      startAt: start,
      endAt: end,
      metadata: { target_margin_percent: targetMarginPercent },
    });
    try {
  ...
  if (!dryRun && recalculate) {
    usageRecalc = await recalculateUsageEventCosts({
      organizationId,
      start,
      end,
      force: true,
    ...
    try {
      ...
      warnings.push(
        `daily rollup rebuild skipped: ${err.message || String(err)}`,
      );
    }
  }
  if (!dryRun && recalculateCustomers) {
    customerRecalc = await recalculateCustomerCharges({
      organizationId,
      startAt: start,
      endAt: end,
      applyWallet,
...
async function getVendorRateSyncStatus() {
  ...
  const env = {
    ...
    railway: !!(
      safeString(process.env.RAILWAY_API_TOKEN) &&
      safeString(process.env.RAILWAY_PROJECT_ID)
    ),
    supabase: !!(
      safeString(process.env.SUPABASE_ACCESS_TOKEN) &&
      safeString(
        process.env.SUPABASE_PROJECT_REF || process.env.SUPABASE_PROJECT_ID,
      )
    ),
  ...
  try {
    const { data } = await sb
      .from("billing_admin_vendor_rate_sync_status_v55")
      .select("*")
      .limit(100);
    latestRuns = data || [];
  } catch (_) {}
  try {
    const { data } = await sb
      .from("billing_admin_unpriced_usage_v55")
      .select("*")
      .limit(100);
    unpricedUsage = data || [];
  } catch (_) {}
  try {
    const { data } = await sb
      .from("billing_admin_rate_card_coverage_v54")
      .select("*")
      .limit(100);
    rateCoverage = data || [];
  } catch (_) {}
...
async function getMarginRiskReport({
  organizationId = null,
  hours = 24,
  ...
  // V59: do not rely on PostgREST embedded relationships between
  // billing_customer_usage_charges and billing_usage_events. Several Supabase
  // projects do not expose that FK relationship in the schema cache, which made
  // the margin-risk endpoint fail with:
  // "Could not find a relationship between billing_customer_usage_charges and billing_usage_events".
  let chargeQuery = sb
  ...
  const usageEventIds = Array.from(
    new Set((charges || []).map((row) => row.usage_event_id).filter(Boolean)),
  );
  const usageEventsById = new Map();
  const chunkSize = 500;
  for (let i = 0; i < usageEventIds.length; i += chunkSize) {
    const ids = usageEventIds.slice(i, i + chunkSize);
    const { data: events, error: eventError } = await sb
      .from("billing_usage_events")
      .select(
        "id,provider,service,event_type,unit,occurred_at,call_id,voice_agent_id,knowledge_base_id,metadata",
      )
      .in("id", ids)
      .limit(chunkSize);
    if (eventError) throw eventError;
    for (const event of events || []) usageEventsById.set(event.id, event);
  }

  const target = Math.max(0, Math.min(99, safeNumber(targetMarginPercent, 70)));
  const rows = (charges || []).map((row) => {
    const usageEvent = row.usage_event_id
      ? usageEventsById.get(row.usage_event_id)
      : null;
    ...
    if (charge <= 0 && cost > 0) decision = "unbilled_cost";
      ...
      decision = "margin_below_target";
    return {
      id: row.id,
      organizationId: row.organization_id,
      usageEventId: row.usage_event_id,
      provider: row.provider || usageEvent?.provider || null,
      service: row.service || usageEvent?.service || null,
      eventType: row.event_type || usageEvent?.event_type || null,
      unit: row.unit || usageEvent?.unit || null,
      quantity: safeNumber(row.quantity),
      customerChargeUsd: charge,
      internalCostUsd: cost,
      grossProfitUsd: profit,
      grossMarginPercent:
        margin == null ? null : Math.round(margin * 100) / 100,
      decision,
      chargedAt: row.charged_at,
      occurredAt: usageEvent?.occurred_at || null,
      callId: usageEvent?.call_id || null,
      voiceAgentId: usageEvent?.voice_agent_id || null,
      knowledgeBaseId: usageEvent?.knowledge_base_id || null,
      relationshipSource: usageEvent
        ? "manual_usage_event_lookup_v59"
        : "charge_only_no_usage_event",
    };
  });
  ...
  summary.grossMarginPercent =
    summary.customerChargeUsd > 0
      ? Math.round(
          (summary.grossProfitUsd / summary.customerChargeUsd) * 10000,
        ) / 100
      : null;
  return {
    organizationId,
...
async function getRecommendedCustomerPricing({
  organizationId = null,
  hours = 24,
  targetMarginPercent = 70,
  startAt = null,
  ...
  let query = sb
    .from("billing_usage_events")
    .select(
      "provider,service,event_type,unit,quantity,estimated_cost_usd,organization_id,occurred_at",
    )
  ...
  if (error) throw error;
  const target =
    Math.max(0, Math.min(95, safeNumber(targetMarginPercent, 70))) / 100;
  ...
  const rows = Array.from(groups.values())
    .map((item) => {
      const costPerUnit =
        item.quantity > 0 ? item.internalCostUsd / item.quantity : 0;
      const minimumCustomerUnitPrice =
        target < 1 ? costPerUnit / (1 - target) : null;
      return {
        ...item,
        internalCostPerUnitUsd: Math.round(costPerUnit * 100000000) / 100000000,
        targetMarginPercent: Math.round(target * 10000) / 100,
        minimumCustomerUnitPriceUsd:
          minimumCustomerUnitPrice == null
            ? null
            : Math.round(minimumCustomerUnitPrice * 100000000) / 100000000,
      };
    })
  ...
  return {
    ...
    startAt: start,
    endAt: end,
    targetMarginPercent: target * 100,
    rows,
  };
}
