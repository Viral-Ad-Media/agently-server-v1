...
async function updateCallRecordBySid(callSid, patch) {
  ...
  const { data, error } = await getSupabase()
    ...
    .maybeSingle();
  if (error)
    console.warn("[call-records] update by sid failed:", error.message);
  return data || null;
}
      ...
      messages: [
        {
          role: "system",
          content:
            'Extract call CRM data. Return only JSON: {"lead":{"name":"","phone":"","email":"","reason":""},"unanswered": [{"question":"","bot_response":""}],"needs_followup": true|false}. If absent use empty strings/array.',
        },
        {
          role: "user",
          content: `Caller phone: ${record.caller_phone || ""}\nSummary: ${summary || ""}\n\nTranscript:\n${text.slice(0, 12000)}`,
        },
      ],
    });
...
async function createLeadFromCall(record, extracted) {
  ...
  const email = String(lead.email || "")
    ...
    .toLowerCase();
  const reason = String(
    lead.reason || record.summary || "Voice call follow-up",
  ).slice(0, 1000);
  if (
    !record.organization_id ||
    (!phone && !email && !extracted?.needs_followup)
  )
    return null;
  const db = getSupabase();
  const row = {
    organization_id: record.organization_id,
    voice_agent_id: record.voice_agent_id || null,
    name:
      String(lead.name || record.caller_name || "Unknown Caller").trim() ||
      "Unknown Caller",
    phone,
    email,
    reason,
    status: "new",
    source: record.direction === "web" ? "web_voice" : "call",
    tags: ["voice-call", record.direction || "inbound"],
    assignment_context: `Created automatically from ${record.direction || "voice"} call ${record.twilio_call_sid || record.id}`,
  };
  const { data, error } = await db
    ...
    .maybeSingle();
  if (error) {
    console.warn("[call-records] lead insert failed:", error.message);
    return null;
  }
...
async function saveUnansweredFromCall(record, extracted) {
  const items = Array.isArray(extracted?.unanswered)
    ? extracted.unanswered
    : [];
  ...
  const rows = items
    ...
    .map((x) => ({
      ...
      bot_response: String(
        x.bot_response ||
          "Captured from voice call; assistant could not fully answer.",
      ).slice(0, 2000),
      is_resolved: false,
    }));
  if (rows.length)
...
async function finalizeCallRecord({
  ...
  if (!summary && transcript.length) {
    try {
      summary = await generateCallSummary(transcriptText(transcript), status);
    } catch (_) {
      summary = "Call completed.";
    }
  }
  const extracted = await extractCallFollowup({
    record: { ...record, summary },
    summary,
  });
  let createdLead = null;
  if (!record.lead_id)
    createdLead = await createLeadFromCall({ ...record, summary }, extracted);
  const finalLeadId = record.lead_id || createdLead?.id || null;
  ...
  const duration =
    Number.isFinite(durationSeconds) && durationSeconds > 0
      ? durationSeconds
      : calculatedDuration;
  const updated = await updateCallRecordById(record.id, {
    status,
    outcome: status,
    duration,
    summary,
  ...
  if (finalLeadId) {
    // N008, second instance. This used to set crm_stage = appointment_set
    // whenever the status or the AI-GENERATED SUMMARY contained the word
    // "appointment" — so a summary reading "the caller declined an
    // appointment" marked the lead as booked. Nothing here has any knowledge
    // of a calendar, and no booking system exists (a5), so this cannot
    // truthfully assert a booking. A completed call means the lead was
    // contacted; that is all that is known.
    const nextStage = "contacted";
    try {
      await db
        .from("leads")
        .update({
        ...
        .in("crm_stage", ["new", "contacted"]);
    } catch (e) {
      console.warn("[call-records] lead CRM update skipped:", e.message);
    }
    try {
      await recordLeadActivity(db, {
        ...
        activityType: "call",
        title: `Call ${status}`,
        body: summary || transcriptText(transcript).slice(0, 2000),
        channel: "voice",
        direction: record.direction || "inbound",
        provider: record.provider || "twilio",
        providerEventId: record.twilio_call_sid || null,
        callId: record.id,
        voiceAgentId: record.voice_agent_id || null,
        metadata: { duration, end_reason: endReason, outcome: status },
        occurredAt: nowIso(),
      });
    } catch (e) {
      console.warn("[call-records] lead activity insert skipped:", e.message);
    }
  }
