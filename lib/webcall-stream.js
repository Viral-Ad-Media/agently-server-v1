...
async function handleWebcallStreamWS(browserWs, req) {
  ...
  if (!apiKey) {
    safeSend(browserWs, {
      ...
      message:
        "Voice calling is temporarily unavailable. Please try again shortly.",
    });
    try {
  ...
  if ((activeSessionsByOrg.get(orgId) || 0) >= WEBCALL_MAX_CONCURRENT_PER_ORG) {
    safeSend(browserWs, {
      ...
      code: "SESSION_LIMIT",
      message:
        "Only one live call can run at a time. End the other session and try again.",
    });
    try {
  ...
  try {
    ...
    if (!agentRow) {
      safeSend(browserWs, {
        ...
        message:
          "This agent could not be found. Refresh the page and try again.",
      });
      try {
  ...
  function markReady(reason) {
    ...
    if (readyTimer) clearTimeout(readyTimer);
    console.log(
      `[webcall-stream] ready (${reason}) agent=${agentId} voice=${openAiVoice} provider=${useElevenLabs ? "elevenlabs" : "openai"}`,
    );
    safeSend(browserWs, {
      type: "session.ready",
      agentName,
  ...
  openaiWs.on("message", (raw) => {
    ...
    if (type === "error") {
      ...
      console.error(
        `[webcall-stream] OpenAI error agent=${agentId}:`,
        JSON.stringify(err),
      );
      return;
    }
