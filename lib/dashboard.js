 ...
 */
/**
 * ISSUE 10 - "Chatbot answers 0 / 0 messages is static. I have asked the
 * chatbot a question and it answers, yet nothing updates here."
 *
 * Correct: this module only ever counted chatbot LEADS. The dashboard reads
 * chatbot.messagesAnswered and chatbot.totalMessages, neither of which was
 * produced here, so both resolved to undefined and rendered as 0 permanently.
 *
 * Counted from the messages themselves: assistant rows are answers, all rows
 * are the total.
 */
function summarizeChatbotMessages(chatMessages = []) {
  const rows = Array.isArray(chatMessages) ? chatMessages : [];
  const roleOf = (m) =>
    String(m?.role || m?.sender || m?.author || "").toLowerCase();
  const answered = rows.filter((m) =>
    ["assistant", "bot", "agent", "ai"].includes(roleOf(m)),
  ).length;
  return { messagesAnswered: answered, totalMessages: rows.length };
}

function buildDashboard(org, calls, leads, activeAgent, chatMessages = []) {
  ...
  return {
    stats: {
      ...
      ...summarizeChatbotMessages(chatMessages),
      callLeadsCaptured,
      missedCalls,
