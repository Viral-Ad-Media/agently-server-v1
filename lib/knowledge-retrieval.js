...
async function searchScopedKnowledgeChunks(db, options = {}) {
  ...
  return fallbackSearchChunks(db, {
    ...
    voiceAgentId: options.voiceAgentId || null,
    linkedChatbotIds: asIdArray(options.linkedChatbotIds),
  });
}
...
async function searchScopedFaqs(db, options = {}) {
  ...
  } else if (options.voiceAgentId) {
    builder = builder.eq("voice_agent_id", options.voiceAgentId);
  } else if (options.chatbotId) {
    // The faqs table is Knowledge Base scoped. Some live schemas do not have a
    // chatbot_id column, so never query it here. Without a selected Knowledge
    // Base, return no chatbot FAQ fallback rather than leaking legacy FAQs from
