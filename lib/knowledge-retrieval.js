...
async function searchScopedFaqs(db, options = {}) {
  ...
  let builder = db
    .from("faqs")
    .select(
      "id,question,answer,voice_agent_id,chatbot_id,knowledge_base_id,knowledge_source_id,metadata,updated_at,created_at",
    )
    .eq("organization_id", organizationId)
  ...
  if (knowledgeBaseIds.length) {
    ...
    builder = builder.eq("chatbot_id", options.chatbotId);
  }
