...
// Last-resort fallback for getApiBaseUrl(). It is normally unreachable — the
// request-derived host wins first — but whatever it resolves to gets BAKED INTO
// the embed snippet a customer pastes onto their site, where it cannot be
// corrected later. It pointed at agently-server-v1.vercel.app, a host being
// retired, so any snippet generated from it would have died with that project.
...
function getRequestBaseUrl(req) {
  ...
  if (!host) return "";
  return cleanBaseUrl(
    `${String(proto).split(",")[0]}://${String(host).split(",")[0]}`,
  );
}
...
function buildResponsiveEmbed(chatbot, widgetUrl) {
  ...
  const safeName = safeComment(
    chatbot.name || chatbot.header_title || chatbot.headerTitle || "My Chatbot",
  );
