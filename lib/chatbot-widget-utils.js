...
function getApiBaseUrl(req) {
  const configured = cleanBaseUrl(process.env.API_URL || process.env.PUBLIC_API_URL || process.env.BACKEND_URL);
  if (configured) return configured;
  if (req) {
    const proto = req.headers["x-forwarded-proto"] || req.protocol || "https";
    const host = req.headers["x-forwarded-host"] || req.headers.host;
    if (host) return `${String(proto).split(",")[0]}://${String(host).split(",")[0]}`.replace(/\/+$/, "");
  }
  return "";
}
