...
function preflightEnabled() {
  return (
    String(process.env.AI_PROVIDER_PREFLIGHT_ENABLED || "true")
      .trim()
      .toLowerCase() !== "false"
  );
}
...
function preflightEnforced() {
  return [
    process.env.AI_PROVIDER_PREFLIGHT_ENFORCE,
    process.env.AI_PROVIDER_PREFLIGHT_REQUIRED,
    process.env.AI_PROVIDER_REQUIRED,
  ].some(
    (value) =>
      String(value || "")
        .trim()
...
function classifyOpenAIError(raw) {
  ...
  if (
    /insufficient[_\s-]?quota|quota|billing|exceeded your current quota/.test(
      text,
    )
  ) {
    return {
  ...
  if (
    /invalid[_\s-]?api[_\s-]?key|incorrect api key|unauthorized|401/.test(text)
  ) {
    return {
  ...
  if (
    /model[_\s-]?not[_\s-]?found|model .* not found|does not exist|404/.test(
      text,
    )
  ) {
    return {
  ...
  if (
    /billing[_\s-]?not[_\s-]?active|payment required|billing inactive/.test(
      text,
    )
  ) {
    return {
...
async function checkOpenAIRealtimeProvider() {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.warn(
        `[ai-provider] preflight failed reason=${classified.reason}`,
      );
    ...
    const reason =
      err?.name === "AbortError"
        ? "provider_timeout"
        : classifyOpenAIError(err?.message || err).reason;
    const message =
