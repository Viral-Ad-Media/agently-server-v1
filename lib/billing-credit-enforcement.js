...
async function getWalletCreditStatus({
  ...
  // Customer-facing usage must not proceed below the active minimum, even when
  // the legacy env still says observe. Observe/warn only remain visible labels
  // for configuration compatibility; the runtime decision is now strict.
  const shouldBlock = hasReachedHardStop || !hasEnoughCredit;
