...
/**
 * PLATFORM ASSISTANT — Agently's own workspace runs outside the credit wall so
 * tenants can ask the in-app assistant unlimited questions without being
 * charged for support.
 *
 * The env allow-list is kept as the first check because it needs no database
 * round-trip and still works if the platform migration has not been applied in
 * this environment. The DB flag (organizations.is_platform_org / billing_exempt)
 * is the durable source of truth: env vars drift silently between Vercel,
 * production and local, and an exemption that only half-exists is worse than none.
 *
 * Exemption is NOT the same as "uncapped". The platform organization has its
 * own daily spend ceiling enforced in lib/platform-assistant.js.
 */
async function isOrgExempt(organizationId) {
  ...
  try {
    const { isPlatformOrganization } = require("./platform-assistant");
    return await isPlatformOrganization(orgId);
  } catch (_) {
    // Module missing (pre-migration deploy) — fall back to env only.
    return false;
  }
}
