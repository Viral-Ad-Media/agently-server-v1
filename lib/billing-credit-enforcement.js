...
function enforcementMode() {
  ...
  return ["observe", "warn", "block"].includes(mode) ? mode : "block";
}
...
function minimumForAction(action, override) {
  ...
  return Math.max(platformMinimum, defaults[key] ?? defaults.usage);
}
...
async function getWalletCreditStatus({
  ...
  const shouldBlock = !hasEnoughCredit;
