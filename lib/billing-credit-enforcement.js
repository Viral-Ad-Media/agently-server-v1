...
async function getWalletCreditStatus({
  ...
  // Observe/warn modes can allow controlled testing below zero, but the hard-stop
  // threshold always blocks. This prevents silent runaway spend when a tenant has
  // no credit but enforcement is not fully switched to block yet.
  const shouldBlock =
    hasReachedHardStop || (mode === "block" && !hasEnoughCredit);
