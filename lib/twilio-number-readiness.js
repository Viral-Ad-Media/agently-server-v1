...
function autoReadyLowRiskVoiceEnabled() {
  return (
    String(
...
function shouldAutoReadyVoice(number, capabilities) {
  if (!autoReadyLowRiskVoiceEnabled()) return false;
  if (!truthy(capabilities?.voice)) return false;
  const country = normalizeCountry(number?.iso_country || "");
  return (
    !country ||
    country === "TWILIO" ||
    lowRiskVoiceCountries().includes(country) ||
    String(number?.phone_number || "").startsWith("+1")
  );
}
...
async function getTwilioNumberReadiness(numberId, options = {}) {
  ...
  const inbound_voice = !supportsVoice
    ...
    : shouldAutoReadyVoice(number, capabilities)
      ? check(
          "ready",
          "US/low-risk voice-capable numbers are auto-ready for inbound voice in this deployment.",
          { autoReady: true },
        )
  ...
  const outbound_voice = !supportsVoice
    ...
    : shouldAutoReadyVoice(number, capabilities)
      ? check(
          "ready",
          "US/low-risk voice-capable numbers are auto-ready for outbound voice in this deployment.",
          {
            countries: voiceCountries.length
              ? voiceCountries
              : lowRiskVoiceCountries(),
            autoReady: true,
          },
        )
      : missingVoice.length
