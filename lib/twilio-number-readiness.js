...
function defaultLowRiskVoiceCountries() {
  const raw =
...
async function getTwilioNumberReadiness(numberId, options = {}) {
  ...
  const autoReadyCountries = autoReadyLowRiskVoiceEnabled()
    ? defaultLowRiskVoiceCountries()
    : [];
  const voiceCountries = [
    ...new Set(
      [
        homeCountry,
        ...selectedVoiceCountries.map(normalizeCountry),
        ...autoReadyCountries,
      ].filter(Boolean),
    ),
  ];
  ...
  const outbound_voice = !supportsVoice
    ...
    : missingVoice.length
      ...
      : check(
          "ready",
          "Outbound voice countries are enabled or auto-confirmed for low-risk testing.",
          {
            countries: voiceCountries,
            autoReadyLowRiskVoice: autoReadyLowRiskVoiceEnabled(),
          },
        );
