...
function csvCountries(value, fallback = "US") {
...
function autoReadyLowRiskVoiceNumbers() {
  return boolEnv("AUTO_READY_LOW_RISK_VOICE_NUMBERS", true);
}
...
function isLowRiskVoiceCountry(country) {
  return lowRiskVoiceCountries().has(normalizeCountry(country || "US"));
}
...
async function getTwilioNumberReadiness(numberId, options = {}) {
  ...
  const autoReadyVoice = autoReadyLowRiskVoiceNumbers() && supportsVoice;
  ...
  const inbound_voice = !supportsVoice
    ...
    : autoReadyVoice
      ? check(
          "ready",
          "Inbound voice is auto-ready for owned US/low-risk voice-capable numbers.",
        )
  ...
  const allCountriesLowRisk = (
    voiceCountries.length ? voiceCountries : ["US"]
  ).every(isLowRiskVoiceCountry);

  const outbound_voice = !supportsVoice
    ? check("not_supported", "This number does not support outbound voice.")
    : autoReadyVoice && allCountriesLowRisk
      ? check(
          "ready",
          "Outbound voice is auto-ready for US/low-risk destinations.",
          {
            countries: voiceCountries.length ? voiceCountries : ["US"],
          },
        )
      : missingVoice.length
        ? check(
