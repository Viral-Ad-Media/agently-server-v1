...
/**
 * What actually stopped us reading this page, and what the tenant should do.
 *
 * Both fetch attempts are considered together. Reporting only the last failure
 * meant a site that returns 403 to us but merely rate-limits our reader was
 * described as a reader problem, and a site we simply could not render was
 * described as hostile.
 */
const SCRAPE_FAILURE = {
  site_blocked: {
    reason:
      "This site blocks automated readers, so we could not open it.",
    action:
      "Upload a brochure, price list or service PDF instead, or paste the key pages in as FAQs. Hand-written answers outrank scraped pages anyway.",
  },
  javascript_only: {
    reason:
      "This site builds its pages with JavaScript, so there is nothing to read in the page source.",
    action:
      "Try again in a few minutes — our reader handles these, it was just unavailable. If it keeps failing, upload a PDF or add the key answers as FAQs.",
  },
  reader_rate_limited: {
    reason:
      "We hit our own reader's rate limit before we got to this page. Nothing is wrong with your website.",
    action:
      "Retry this source in a few minutes. Fewer, better pages usually beats a full crawl.",
  },
  page_timeout: {
    reason: "The page took too long to respond.",
    action: "Retry this page on its own. If it always times out, the site may be slow for everyone.",
  },
  not_found: {
    reason: "That page does not exist any more.",
    action: "Remove it from the list, or point the source at your sitemap so we follow live links only.",
  },
  thin_page: {
    reason: "The page loaded but had almost no readable text on it.",
    action: "Point us at a page with real content — services, pricing, about, FAQs — rather than a landing or splash page.",
  },
  unreachable: {
    reason: "We could not reach that address at all.",
    action: "Check the URL is public and spelled correctly, and that the site is up.",
  },
};
...
/**
 * @param rawError   what the direct fetch threw, if anything
 * @param readerError what r.jina.ai threw, if we got that far
 */
function classifyScrapeFailure(rawError, readerError) {
  const raw = cleanText(rawError?.message || rawError || "");
  const reader = cleanText(readerError?.message || readerError || "");
  const rawStatus = statusOf(raw);
  const readerStatus = statusOf(reader);

  // Ours before theirs. A 429 from the reader says nothing about their site,
  // and blaming them for it is the failure mode this whole function exists for.
  if (readerStatus === 429 || /rate.?limit|too many requests/i.test(reader)) {
    return { code: "reader_rate_limited", ...SCRAPE_FAILURE.reader_rate_limited };
  }

  if (rawStatus === 401 || rawStatus === 403 || /captcha|cloudflare|access denied|forbidden/i.test(raw)) {
    return { code: "site_blocked", ...SCRAPE_FAILURE.site_blocked };
  }
  if (rawStatus === 404 || readerStatus === 404) {
    return { code: "not_found", ...SCRAPE_FAILURE.not_found };
  }

  // The direct fetch worked but produced nothing worth keeping, which is what a
  // client-rendered page looks like from the outside. If the reader also could
  // not deliver, say that plainly rather than calling the site hostile.
  if (/too little text/i.test(raw)) {
    if (reader) {
      return { code: "javascript_only", ...SCRAPE_FAILURE.javascript_only };
    }
    return { code: "thin_page", ...SCRAPE_FAILURE.thin_page };
  }

  if (/timeout|aborted|AbortError/i.test(raw) || /timeout|aborted|AbortError/i.test(reader)) {
    return { code: "page_timeout", ...SCRAPE_FAILURE.page_timeout };
  }
  if (/ENOTFOUND|EAI_AGAIN|ECONNREFUSED|getaddrinfo/i.test(raw + " " + reader)) {
    return { code: "unreachable", ...SCRAPE_FAILURE.unreachable };
  }

  return {
    code: "unknown",
    reason: raw || reader || "Unknown error",
    action: "Retry this page on its own, or add the key answers as FAQs.",
  };
}

function failureReason(error) {
  // fetchPage attaches the classification it worked out while it still had
  // both attempts in hand; prefer it over re-guessing from a flattened string.
  if (error && typeof error === "object" && error.scrapeDiagnosis) {
    return error.scrapeDiagnosis.reason;
  }
  return classifyScrapeFailure(error, null).reason;
}

function failureCode(error) {
  if (error && typeof error === "object" && error.scrapeDiagnosis) {
    return error.scrapeDiagnosis.code;
  }
  return classifyScrapeFailure(error, null).code;
}
...
function buildScrapeReport({
  ...
  const failedPages = pageFailures.slice(0, 30).map((item) => ({
    ...
    // Machine-readable, so the UI can group "our fault, retry" separately from
    // "your site needs a different approach" instead of parsing prose.
    code: failureCode(item.reason || item.error || item.message),
  }));
  ...
  /*
   * Advice derived from the failures we actually saw. The old pair of generic
   * lines was shown even when the only problem was our own reader's quota,
   * which sent tenants off to rewrite their website by hand for no reason.
   */
  const suggestedActions = [];
  const codes = new Set(
    pageFailures.map((item) => failureCode(item.reason || item.error || item)),
  );
  for (const code of ["reader_rate_limited", "site_blocked", "javascript_only", "not_found", "page_timeout", "thin_page", "unreachable"]) {
    if (codes.has(code) && SCRAPE_FAILURE[code]) {
      suggestedActions.push(SCRAPE_FAILURE[code].action);
    }
  }
  if (codes.has("unknown")) {
    suggestedActions.push(
      "Retry failed pages individually from the Knowledge Base page.",
    );
  }
  /*
   * Only when access is genuinely implicated.
   *
   * This line used to appear on every empty crawl. It sent tenants off to audit
   * a website that was never the problem when our own reader had run out of
   * quota, and when the site simply renders client-side — which is not blocking
   * at all, and is the most common case of the three.
   */
  const accessMightBeTheProblem =
    codes.has("site_blocked") || codes.has("unknown") || codes.size === 0;
  if (!scraped && accessMightBeTheProblem) {
    suggestedActions.push(
...
async function fetchText(
  ...
  // safeFetch, not fetch. This URL came from a tenant, and redirect:"follow"
  // meant a public URL could bounce us into the private network with nothing
  // re-checking the destination. Every hop is now resolved and validated, and
  // the socket is pinned to the address that was checked. See lib/safe-fetch.js.
  return safeFetch(url, { accept, timeoutMs, userAgent: USER_AGENT });
}
...
async function fetchPage(url) {
  ...
  try {
    ...
    if (process.env.SCRAPER_VERBOSE_LOGS === "true") {
      console.warn(
        "[scraper-v2] raw fetch failed, trying reader:",
        normalized,
        rawErr.message,
      );
    }
    try {
      ...
      /*
       * Both attempts have now failed, and only here do we still know how each
       * one failed. Previously the reader's error simply replaced the raw one,
       * so a site returning 403 while the reader merely ran out of quota was
       * reported as a reader problem — and, far worse, a reader quota failure
       * was reported to the tenant as their own website blocking us.
       */
      const diagnosis = classifyScrapeFailure(rawErr, readerErr);
      const error = new Error(diagnosis.reason);
      error.scrapeDiagnosis = diagnosis;
      error.rawFetchError = cleanText(rawErr?.message || "");
      error.readerError = cleanText(readerErr?.message || "");
      throw error;
    }
  }
}
