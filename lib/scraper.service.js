...
function classifyContentKind(page = {}) {
...
${text.slice(0, 6000)}`.toLowerCase();
  ...
  const productSignals = [
    ...
    /\/cart\/?/i,
    /\/checkout\/?/i,
    /\b(add to cart|buy now|shop now|order now|checkout|in stock|out of stock|sku|quantity|variant|sale price|regular price)\b/i,
    /\b(product|supplement|capsule|tablet|gummy|gummies|serum|cream|bundle|pack|bottle|dose|serving|ingredients?|nutrition facts?)\b/i,
    /(?:\$|£|€|₦|usd|gbp|eur|ngn|cad|aud)\s?\d+[\d,.]*/i,
    /\d+[\d,.]*\s?(?:usd|gbp|eur|ngn|cad|aud)/i,
  ];
  const serviceSignals = [
    /\/services?\//i,
    /\/pricing\/?/i,
    /\/book(?:ing)?\/?/i,
    /\b(book a call|book now|schedule|appointment|consultation|quote|estimate|service|services|package|plan|pricing|subscription)\b/i,
  ];

  const productScore = productSignals.reduce(
    (score, pattern) => score + (pattern.test(haystack) ? 1 : 0),
    0,
  );
  const serviceScore = serviceSignals.reduce(
    (score, pattern) => score + (pattern.test(haystack) ? 1 : 0),
    0,
  );

  // DTC and blog-style product pages often avoid /product/ URL paths, so do not
  // require a product URL. Product ranking is only a tie-breaker later, not a hard
  // filter, so a generous classifier is safer than burying real product pages.
  if (
    productScore >= 2 ||
    (productScore >= 1 &&
      /price|buy|order|cart|stock|ingredient|supplement|gumm/i.test(haystack))
  ) {
    return "product";
  }
...
async function scrapeAndStore({
  ...
  try {
    ...
    for (const p of result.pages) {
      const contentKind = classifyContentKind(p);
      pageChunks.push(...chunkText(p.text, p.url, p.title, contentKind));
    }
