...
function warnKnowledgeBaseIssue(label, error) {
  const key = `${label}:${String(error?.code || error?.message || error || "unknown")}`;
  const now = Date.now();
...
function serializeKnowledgeBase(row, extras = {}) {
  ...
  return {
    ...
    // FIX: "That doesn't look like a valid website address."
    // The column is primary_url; there is no `website` column and never was.
    // The Knowledge Base page read `.website`, got undefined, and sent an empty
    // string to /discover — which correctly rejected it. Aliased here (falling
    // back to the primary source URL) so every caller has one field to read.
    website:
      row.primary_url ||
      (extras.sources || []).find((src) => src?.is_primary)?.url ||
      (extras.sources || [])[0]?.url ||
      "",
    ...
    // Fields consumed by the discovery / monitoring UI.
    latestDiscoveryId: extras.latestDiscoveryId || null,
    changeMonitoringEnabled: row.change_monitoring_enabled === true,
    changeMonitoringMode: row.change_monitoring_mode || "auto_rescrape",
    changeMonitoringIntervalHours: row.change_monitoring_interval_hours || 24,
    lastChangeCheckAt: row.last_change_check_at || null,
  ...
  try {
    ...
    if (error) {
      if (!isMissingTableError(error)) {
        warnKnowledgeBaseIssue(label, error);
      }
      return fallback;
    }
    ...
    if (!isMissingTableError(e)) {
      warnKnowledgeBaseIssue(label, e);
    }
    return fallback;
  }
}
...
async function listKnowledgeBasesForOrg(db, organizationId) {
  ...
  const [
    ...
    safeDb(
      "list latest page discoveries",
      () =>
        db
          .from("knowledge_page_discoveries")
          .select("id,knowledge_base_id,status,created_at")
          .eq("organization_id", organizationId)
          .in("knowledge_base_id", ids)
  ...
  const latestDiscoveryByKnowledgeBase = new Map();
  for (const discovery of discoveries || []) {
    if (
      discovery?.knowledge_base_id &&
      !latestDiscoveryByKnowledgeBase.has(discovery.knowledge_base_id)
    ) {
      latestDiscoveryByKnowledgeBase.set(
        discovery.knowledge_base_id,
        discovery.id,
      );
    }
  ...
  return rows.map((row) => {
    ...
    return serializeKnowledgeBase(row, {
      ...
      latestDiscoveryId: latestDiscoveryByKnowledgeBase.get(row.id) || null,
    });
  });
}
...
async function getAssignedKnowledgeBaseIdsForVoiceAgent(
  ...
  const directId = agent?.knowledge_base_id || null;
  if (directId) return [directId];

  const ignoredLinks = await safeDb(
    "voice legacy KB links diagnostic",
    () =>
      db
        .from("agent_knowledge_base_links")
  ...
  if ((ignoredLinks || []).length) {
    console.warn(
      "[kb-scope] voice legacy KB links ignored; direct voice_agents.knowledge_base_id is required",
      {
        organizationId,
        agentId,
  ...
  // Strict KB isolation: no direct assignment means no business KB context.
  return [];
}
...
async function getAssignedKnowledgeBaseIdsForChatbot(
  ...
  const ignoredLinks = await safeDb(
    "chatbot legacy KB links diagnostic",
    () =>
      db
        .from("chatbot_knowledge_base_links")
        .select("knowledge_base_id")
  ...
  if ((ignoredLinks || []).length) {
    console.warn(
      "[kb-scope] chatbot legacy KB links ignored; direct chatbots.knowledge_base_id is required",
      {
        organizationId,
        chatbotId,
        ignoredLinkCount: ignoredLinks.length,
