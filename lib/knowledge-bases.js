...
async function getAssignedKnowledgeBaseIdsForVoiceAgent(
  ...
  const primaryLink = (links || []).find((l) => l.knowledge_base_id) || null;
  const directId = agent?.knowledge_base_id || null;
  const linkId = primaryLink?.knowledge_base_id || null;
  if (directId && linkId && directId !== linkId) {
    console.warn(
      "[kb-scope] voice KB assignment mismatch; direct agent value wins",
      {
        organizationId,
        agentId,
        directId,
        ...
        linkCreatedAt: primaryLink?.created_at || null,
      },
    );
  }
