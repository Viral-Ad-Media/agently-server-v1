...
function titleFromDomain(domain) {
  const base = cleanText(domain).replace(/^www\./, "").split(".")[0] || "Business";
  return base
    .replace(/[-_]+/g, " ")
...
function serializeKnowledgeBase(row, extras = {}) {
  ...
  return {
    ...
    name: row.name || row.business_name || "Business Knowledge Base",
    businessName: row.business_name || row.name || "Business Knowledge Base",
...
async function ensureDefaultKnowledgeBaseForOrg(db, org) {
  ...
  const businessName = cleanText(org.name) || titleFromDomain(domain) || "Primary Business";
    ...
    () =>
      db
        ...
        .insert({
          ...
          description: "Default business knowledge base created from onboarding profile.",
  ...
  if (row?.id) await ensurePrimarySource(db, { organizationId, knowledgeBase: row });
  return row;
}
...
async function listKnowledgeBasesForOrg(db, organizationId) {
  ...
  return rows.map((row) =>
    serializeKnowledgeBase(row, {
      sources: (sources || []).filter((s) => s.knowledge_base_id === row.id),
      linkedVoiceAgentIds: (agentLinks || [])
        .filter((l) => l.knowledge_base_id === row.id)
        .map((l) => l.voice_agent_id),
      linkedChatbotIds: (chatbotLinks || [])
        .filter((l) => l.knowledge_base_id === row.id)
        .map((l) => l.chatbot_id),
    }),
  );
...
async function getAssignedKnowledgeBaseIdsForVoiceAgent(
  ...
  const fallback = await ensureDefaultKnowledgeBaseForOrg(
    db,
