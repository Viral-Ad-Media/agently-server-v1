...
async function quickLoadAgent(agentId) {
  ...
  try {
    ...
    const { data: agent } = await db
      .from("voice_agents")
      .select(
        "id,organization_id,name,greeting,voice,voice_provider,voice_id,openai_voice,language,tone,knowledge_base_id,business_name,custom_prompt,core_purpose,call_purpose",
      )
      .eq("id", agentId)
      .maybeSingle();
    if (!agent?.id)
...
function buildFastInstructions({
  ...
  return [
    ...
    "Use only the selected Knowledge Base once it is loaded. If you do not have the answer yet, say you need to check the selected knowledge base and offer follow-up instead of inventing.",
