...
function parseKnowledgeSearchArguments(args) {
...
function knowledgeSearchToolDefinition() {
  return {
    type: "function",
    name: "search_business_knowledge",
    description:
      "Search the selected Knowledge Base during the live call for business-specific facts, products, services, prices, policies, booking details, URLs, FAQs, ingredients, availability, shipping, or support answers. Use this before answering a specific question when the loaded prompt does not clearly contain the answer.",
    parameters: {
      type: "object",
      properties: {
        query: {
          type: "string",
          description:
            "The caller's exact question or the concise business topic to search for.",
        },
      },
      required: ["query"],
...
function buildFastInstructions({
  ...
  lines.push(
    ...
    "When a selected Knowledge Base exists, you may use search_business_knowledge silently during the call before answering specific product, service, price, policy, booking, URL, shipping, ingredient, availability, or FAQ questions. Never tell the caller you are using a tool or database.",
...
function buildInstructionsFromLoadedContext({
  ...
  const identityGuard = [
    ...
    "- Use search_business_knowledge silently before answering specific product, service, price, policy, booking, URL, shipping, ingredient, availability, or FAQ questions not clearly covered in the loaded prompt. If search returns nothing useful, offer follow-up instead of inventing.",
  ].join("\n");
...
async function handleOpenAIRealtimeMediaStream(twilioWs, request) {
  ...
  const handledKnowledgeSearchToolCalls = new Set();

  function realtimeToolDefinitions() {
    return primaryKnowledgeBaseId && organizationId
      ? [knowledgeSearchToolDefinition()]
      : [];
  }

  async function handleSearchBusinessKnowledge(rawArgs, callId = "") {
    const toolCallKey = String(callId || "").trim();
    if (toolCallKey && handledKnowledgeSearchToolCalls.has(toolCallKey)) {
      return { success: true, duplicatePrevented: true };
    }
    if (toolCallKey) handledKnowledgeSearchToolCalls.add(toolCallKey);

    const args = parseKnowledgeSearchArguments(rawArgs);
    const query = cleanSpeech(args.query).slice(0, 500);
    const knowledgeBaseIds = primaryKnowledgeBaseId
    ...
    let result = {
      ...
      chunks: [],
      message:
        "No selected Knowledge Base was available for this call. Do not invent an answer; offer to take a message or callback request.",
    };

    if (!query) {
      result.message =
        "No search query was provided. Ask one short clarifying question or offer follow-up.";
    } else if (!organizationId || !knowledgeBaseIds.length) {
      result.message =
        "This call has no selected Knowledge Base scope. Do not search organization-wide; offer follow-up instead.";
    } else {
      try {
        const db = getSupabase();
        ...
        result = {
          ...
          found: (faqs || []).length + (chunks || []).length,
          faqs: (faqs || []).slice(0, 4).map((faq) => ({
            question: faq.question || "",
            answer: faq.answer || "",
            relevance: faq.relevance || 0,
          })),
          chunks: (chunks || []).slice(0, 8).map((chunk) => ({
            ...
            contentKind:
              chunk.content_kind || chunk.metadata?.contentKind || "page",
            score: chunk.searchScore || 0,
          })),
          message:
            (faqs || []).length || (chunks || []).length
              ? "Use these selected-Knowledge-Base results to answer the caller naturally. Include a URL only if it is relevant and present in the result."
              : "No useful selected-Knowledge-Base result found. Do not invent; offer follow-up.",
        };
      } catch (err) {
        console.warn("[knowledge-search] realtime bridge tool failed", {
          callSid: callSid || "",
          streamSid: streamSid || "",
          query,
        ...
        result = {
          ...
          error: err.message || String(err),
          message:
            "Knowledge search failed. Do not invent; offer to take a message or callback request.",
        };
      }
    }
  ...
  function buildRealtimeSessionPayload() {
    return {
      ...
      tool_choice: realtimeToolDefinitions().length ? "auto" : "none",
      audio: {
        input: {
  ...
  openAiWs.on("message", async (raw) => {
    ...
    if (
      ...
      if (name === "search_business_knowledge") {
        void handleSearchBusinessKnowledge(args, toolCallId);
        return;
      }
    }
    ...
    if (event.type === "conversation.item.created") {
      ...
      if (
        ...
        void handleSearchBusinessKnowledge(
          item.arguments,
  ...
  async function finalizeAndClose(reason) {
    ...
    await Promise.allSettled([
      insertUsageEvent({
        organizationId,
        // AWS now, not Railway — see lib/conversation-relay.js for the same
        // change. Railway-era rows keep provider "railway" so the period we
        // actually paid Railway stays legible in the ledger.
        provider: "aws",
        ...
        // Flat monthly bundle, so no per-second rate. Recovered by
        // lib/usage-billing-engine.js as a share of the AWS bill.
        billable: false,
        metadata: {
          call_sid: callSid || null,
          stream_sid: streamSid || null,
