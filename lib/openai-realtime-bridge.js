...
async function loadAgentContext(agentId) {
  ...
  const { data: agent, error } = await db
    ...
    .maybeSingle();
  if (error || !agent)
    throw new Error("Voice agent not found for media stream.");
  ...
  return { agent, faqs: faqs || [], chunks: chunks || [] };
}
...
async function handleOpenAIRealtimeMediaStream(twilioWs, request) {
  ...
  const { agent, faqs, chunks } = await loadAgentContext(agentId);
  const instructions = buildSystemPrompt(agent, faqs, chunks);
    ...
    safeSend(openAiWs, {
      ...
      item: {
        ...
        content: [
          {
            type: "input_text",
            text: `Start the call by saying exactly this greeting, then wait for the caller: ${greeting}`,
          },
        ],
      },
    });
