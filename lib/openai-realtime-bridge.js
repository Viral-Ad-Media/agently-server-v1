...
async function quickLoadAgent(agentId) {
  if (!agentId) return { agent: null, organization: null };
  try {
    const db = getSupabase();
    const { data: agent } = await db
      .from("voice_agents")
      .select(
        "id,organization_id,name,greeting,voice,voice_provider,voice_id,openai_voice,language,tone,knowledge_base_id",
      )
      .eq("id", agentId)
      .maybeSingle();
    if (!agent?.id) return { agent: null, organization: null };
    ...
    return { agent: null, organization: null };
  }
}

function buildFastInstructions({ agent, organization, direction }) {
  return [
    `You are ${agent?.name || "the AI voice agent"} for ${organization?.name || organization?.business_name || organization?.company_name || "this business"}.`,
