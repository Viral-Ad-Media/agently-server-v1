...
async function handleOpenAIRealtimeMediaStream(twilioWs, request) {
  ...
  const { agent, context } = await loadAgentContext(agentId, direction);
  ...
  function initializeRealtimeSession() {
    ...
    const greeting = direction === "outbound"
      ...
      : agent.greeting || "Hello, thank you for calling. How can I help you today?";
    safeSend(openAiWs, {
      type: "conversation.item.create",
      item: {
        type: "message",
        role: "user",
        content: [{ type: "input_text", text: `Speak exactly this greeting and nothing else, then wait for the caller: ${greeting}` }],
      },
    });
