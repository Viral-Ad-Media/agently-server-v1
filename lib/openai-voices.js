...
const OPENAI_TTS_RESPONSE_FORMAT =
  process.env.OPENAI_TTS_RESPONSE_FORMAT || "mp3";
...
const OPENAI_VOICES = [
  {
    ...
    name: "Alloy",
    displayName: "Alloy",
    tone: "Balanced, neutral, versatile",
  },
  { id: "ash", name: "Ash", displayName: "Ash", tone: "Calm, steady, clear" },
  {
    ...
    name: "Ballad",
    displayName: "Ballad",
    tone: "Warm, expressive, storytelling",
  },
  {
    id: "coral",
    name: "Coral",
    displayName: "Coral",
    tone: "Bright, friendly, upbeat",
  },
  {
    id: "echo",
    name: "Echo",
  ...
  },
  {
    ...
    name: "Fable",
    displayName: "Fable",
    tone: "Narrative, warm, characterful",
  },
  {
    id: "nova",
    name: "Nova",
    displayName: "Nova",
    tone: "Friendly, polished, energetic",
  },
  {
    id: "onyx",
    name: "Onyx",
    displayName: "Onyx",
    tone: "Deep, confident, authoritative",
  },
  {
    id: "sage",
    name: "Sage",
    displayName: "Sage",
    tone: "Measured, composed, helpful",
  },
  {
    id: "shimmer",
    name: "Shimmer",
    displayName: "Shimmer",
    tone: "Soft, pleasant, approachable",
  },
  {
    id: "verse",
    name: "Verse",
    displayName: "Verse",
    tone: "Expressive, modern, conversational",
  },
  {
    id: "marin",
    name: "Marin",
    displayName: "Marin",
    tone: "Natural, high-quality, recommended",
  },
  {
    id: "cedar",
    name: "Cedar",
...
async function synthesizeOpenAIPreview({
  ...
  const resolvedModel = normalizeTtsModel(model);
  const resolvedVoice = normalizeVoiceId(
    voiceId,
    process.env.OPENAI_TTS_DEFAULT_VOICE || "alloy",
  );
  if (!supportedByModel(resolvedVoice, resolvedModel)) {
    const err = new Error(
      `Voice '${resolvedVoice}' is not supported by model '${resolvedModel}'.`,
    );
    err.status = 400;
  ...
  const response_format = String(
    responseFormat || OPENAI_TTS_RESPONSE_FORMAT || "mp3",
  ).toLowerCase();
  const body = {
    model: resolvedModel,
    voice: resolvedVoice,
  ...
  if (!response.ok) {
    ...
    const err = new Error(
      `OpenAI speech preview failed with HTTP ${response.status}.`,
    );
  ...
  return {
    ...
    mimeType:
      response.headers.get("content-type") || mimeForFormat(response_format),
