...
/**
 * Flat platform fee per assistant reply, charged on top of the OpenAI tokens.
 *
 * Token margin alone is too thin to make chat a real line: a typical reply
 * costs about $0.0008 in tokens, so even at a 70% margin the platform keeps a
 * fraction of a cent. This fee pays for the surrounding product — retrieval,
 * storage, the widget, support.
 *
 * The PRICE lives in billing_customer_rate_cards as a unit_price row
 * (chatbot/conversation @ $0.01), not here. This event deliberately carries no
 * unit_cost_usd: the fee is revenue, not a vendor cost, so recording it as
 * cost would inflate internal spend and understate margin everywhere it is
 * reported. Change the amount by editing that rate card row — no deploy.
 */
async function logChatbotConversationUsage({ organizationId, userId, chatbotId, messageId, metadata }) {
  return insertUsageEvent({
    ...
    unit: "conversation",
    quantity: 1,
    // Must be billable for the wallet posting step to run at all; it was
    // defaulting true but priced $0.00 because no rate card matched.
    billable: Boolean(organizationId),
    metadata: {
      counted_as: "assistant_response",
      priced_by: "billing_customer_rate_cards chatbot/conversation unit_price",
      ...(metadata || {}),
    },
  });
}
