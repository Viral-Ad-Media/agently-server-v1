...
async function fetchText(
  ...
  // Discovery follows links found on a tenant's pages, so these URLs are even
  // less trusted than the one the tenant typed — a single <a href> pointing at
  // an internal address would have been fetched. safeFetch validates the
  // resolved address on every hop; see lib/safe-fetch.js.
  try {
    const { text } = await safeFetch(url, { timeoutMs, accept, userAgent: UA });
    return text;
  } catch (error) {
    // A blocked request used to look exactly like an empty site: null either
    // way, no log, nothing to alert on. That made the guard both unverifiable
    // from outside and useless as a signal — you could not tell it was working,
    // and you could not see someone probing. Ordinary fetch failures stay quiet
    // because a dead link on a customer's site is not news.
    if (error && error.code === "SSRF_BLOCKED") {
      console.warn(`[page-discovery] SSRF guard refused a target: ${error.message}`);
    }
    return null;
  }
}
