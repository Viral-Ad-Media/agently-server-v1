...
function safeStreamParameterValue(name, value) {
  ...
  const configured = Number(process.env.TWILIO_STREAM_PARAMETER_MAX_CHARS || 450);
  const byNameLimit = Math.max(32, 480 - String(name || "").length);
  const maxChars = Math.max(32, Math.min(Number.isFinite(configured) ? configured : 450, byNameLimit));
...
async function makeOutboundCall({
  ...
  if (detection) params.MachineDetection = detection;
