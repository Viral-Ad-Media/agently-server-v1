...
async function twilioRequest(method, path, params = null, isFullUrl = false) {
  ...
  if (!res.ok) {
    const msg = data?.message || data?.error_message || `Twilio ${res.status} on ${path}`;
    throw new Error(msg);
  }
  return data;
}
 ...
 */
async function searchAvailableNumbers({ country = 'US', type = 'Local', areaCode, contains, limit = 20 }) {
  const sid   = TWILIO_ACCOUNT_SID();
  const token = TWILIO_AUTH_TOKEN();

  // Normalize type capitalization
  const typeMap = { local: 'Local', tollfree: 'TollFree', mobile: 'Mobile', tollFree: 'TollFree' };
  ...
  const params = { PageSize: String(Math.min(limit, 40)), VoiceEnabled: 'true' };
  ...
  try {
    ...
    if (normalizedType === 'Mobile' && (err.message.includes('not found') || err.message.includes('404'))) {
      const fallbackUrl = `${TWILIO_BASE}/Accounts/${sid}/AvailablePhoneNumbers/${country}/Local.json`;
      data = await twilioRequest('GET', fallbackUrl, { PageSize: String(Math.min(limit, 40)), VoiceEnabled: 'true' }, true);
    } else {
      throw err;
    }
  }
 ...
 */
async function purchasePhoneNumber({ phoneNumber, voiceWebhookUrl, statusCallbackUrl, friendlyName }) {
 ...
 */
async function updateNumberWebhooks({ phoneSid, voiceWebhookUrl, statusCallbackUrl }) {
  const apiUrl = (process.env.API_URL || '').replace(/\/$/, '');
  const params = {
  ...
  return twilioRequest('POST', `/IncomingPhoneNumbers/${phoneSid}.json`, params);
}
...
async function listOwnedNumbers() {
  const data = await twilioRequest('GET', '/IncomingPhoneNumbers.json', { PageSize: '100' });
...
function buildConversationRelayTwiml({ agentRow, wsUrl, greeting }) {
  ...
  const welcomeGreeting = greeting || agentRow.greeting || 'Hello, thank you for calling. How can I help you today?';

  // ElevenLabs requires a model attribute, others don't
  const voiceAttr = voice.provider === 'ElevenLabs'
...
async function fetchWhatsAppMessages({ to, from, limit = 50 }) {
  ...
  if (from) params.From = from.startsWith('whatsapp:') ? from : `whatsapp:${from}`;
...
function buildSystemPrompt(agentRow, faqs = [], knowledgeChunks = []) {
  ...
  const captureFields = (agentRow.data_capture_fields || ['name', 'phone', 'email', 'reason']).join(', ');
