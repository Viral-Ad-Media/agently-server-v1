...
function outboundAsyncAmdEnabled(value) {
  if (value === undefined || value === null || value === "") {
    return boolEnv("OUTBOUND_ASYNC_AMD", true);
  }
...
function defaultAmdStatusCallbackUrl() {
  const apiUrl = (process.env.API_URL || "").replace(/\/$/, "");
  return apiUrl ? `${apiUrl}/api/twilio/amd-status` : "";
}
...
async function twilioRequest(
  ...
  const targetAccountSid = accountSidOverride || sid;
  const url = isFullUrl
    ? path
    : `${TWILIO_BASE}/Accounts/${targetAccountSid}${path}`;
  const headers = {
    Authorization: "Basic " + Buffer.from(`${sid}:${token}`).toString("base64"),
    "Content-Type": "application/x-www-form-urlencoded",
  };

  const buildFormBody = (values = {}) => {
  ...
  if (!res.ok) {
    ...
    err.code = data?.code || data?.error_code || res.status;
    err.twilioCode = data?.code || data?.error_code || null;
    err.moreInfo = data?.more_info || data?.moreInfo || null;
    err.details = data;
    err.twilioRequest = {
      method,
      path,
      accountSid: targetAccountSid,
      targetAccountLast4: String(targetAccountSid || "").slice(-4),
    };
    throw err;
  }
  return data;
...
function safeStreamParameterValue(name, value) {
  const raw = String(value ?? "")
    .replace(/[\r\n\t]+/g, " ")
    .replace(/\s+/g, " ")
    .trim();
  const configured = Number(
    process.env.TWILIO_STREAM_PARAMETER_MAX_CHARS || 450,
  );
  const byNameLimit = Math.max(32, 480 - String(name || "").length);
  const maxChars = Math.max(
    32,
    Math.min(Number.isFinite(configured) ? configured : 450, byNameLimit),
  );
  if (raw.length <= maxChars) return raw;
  return raw.slice(0, maxChars - 3).replace(/[\s,;:.-]+$/, "") + "...";
}

/**
 * Generate TwiML to connect a call to a Twilio Media Stream WebSocket.
 * This is used by the backend voice-inbound / voice-test routes that pass
 * contextual parameters to the realtime server.
 */
function buildMediaStreamTwiml({ wsUrl, parameters = {} } = {}) {
  if (!wsUrl) {
    throw new Error("wsUrl is required to build Media Stream TwiML.");
  }
  ...
  const parameterXml = Object.entries(parameters || {})
    ...
    .map(([name, value]) => {
      const safeValue = safeStreamParameterValue(name, value);
      return `      <Parameter name="${escapeXmlValue(name)}" value="${escapeXmlValue(safeValue)}" />`;
    })
    .join("\n");

  const inner = parameterXml ? `\n${parameterXml}\n    ` : "";

  return `<?xml version="1.0" encoding="UTF-8"?>
<Response>
  <Connect>
    <Stream url="${escapeXmlValue(wsUrl)}">${inner}</Stream>
  </Connect>
</Response>`;
}
...
async function makeOutboundCall({
  ...
  const recordingEnabled =
    String(record).toLowerCase() !== "false" &&
    String(process.env.CALL_RECORDING_ENABLED || "true").toLowerCase() !==
      "false";
  const params = {
    From: from,
    To: to,
  ...
  if (detection) {
    params.MachineDetection = detection;
    if (outboundAsyncAmdEnabled(asyncAmd)) {
      params.AsyncAmd = "true";
      const amdCallback =
        amdStatusCallbackUrl ||
        process.env.OUTBOUND_AMD_STATUS_CALLBACK_URL ||
        defaultAmdStatusCallbackUrl();
      if (amdCallback) {
        params.AsyncAmdStatusCallback = amdCallback;
        params.AsyncAmdStatusCallbackMethod = "POST";
      }
    }
    const timeout = Number(
      machineDetectionTimeout ||
        process.env.OUTBOUND_MACHINE_DETECTION_TIMEOUT ||
        0,
    );
    if (Number.isFinite(timeout) && timeout > 0) {
      params.MachineDetectionTimeout = String(timeout);
    }
    const speechThreshold = Number(
      machineDetectionSpeechThreshold ||
        process.env.OUTBOUND_MACHINE_DETECTION_SPEECH_THRESHOLD ||
        0,
    );
    if (Number.isFinite(speechThreshold) && speechThreshold > 0) {
      params.MachineDetectionSpeechThreshold = String(speechThreshold);
    }
    const speechEndThreshold = Number(
      machineDetectionSpeechEndThreshold ||
        process.env.OUTBOUND_MACHINE_DETECTION_SPEECH_END_THRESHOLD ||
        0,
    );
    if (Number.isFinite(speechEndThreshold) && speechEndThreshold > 0) {
      params.MachineDetectionSpeechEndThreshold = String(speechEndThreshold);
    }
    const silenceTimeout = Number(
      machineDetectionSilenceTimeout ||
        process.env.OUTBOUND_MACHINE_DETECTION_SILENCE_TIMEOUT ||
        0,
    );
    if (Number.isFinite(silenceTimeout) && silenceTimeout > 0) {
      params.MachineDetectionSilenceTimeout = String(silenceTimeout);
    }
  }
  if (recordingEnabled && apiUrl) {
    params.Record = "true";
    params.RecordingChannels = "dual";
    params.RecordingStatusCallback = `${apiUrl}/api/twilio/recording-status`;
...
function buildSystemPrompt(agentRow, faqs = [], knowledgeChunks = []) {
  ...
  const lines = [
    `You are ${agentRow?.name || "the AI assistant"}, an AI ${agentRow?.tone || "Professional"} ${agentRow?.direction === "outbound" ? "outbound caller" : "receptionist"} for ${businessName}.`,
