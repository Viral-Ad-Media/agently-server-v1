...
// ─────────────────────────────────────────────────────────────
// Caller ID Verification — for user-owned external numbers
//
// HOW IT WORKS (Twilio Docs):
//   POST /OutgoingCallerIds.json  → Twilio places a call to PhoneNumber,
//     reads a 6-digit validationCode to the recipient.
//     Response is immediate — contains validation_code and call_sid.
//   After the call ends, the number is added to OutgoingCallerIds IF
//     the call was answered and completed successfully.
//   GET  /OutgoingCallerIds.json?PhoneNumber=+xxx → confirms verification.
//
// NOTE: Twilio makes the call in English only. The number must be
//   reachable (not forwarded to voicemail) to successfully verify.
//   International numbers work as long as Twilio can place the call.
//
// Nigerian numbers: use E.164 format e.g. +2349084467821
// ─────────────────────────────────────────────────────────────

/**
 * Start caller ID verification for an external phone number.
 * Twilio places a call to the number and reads a 6-digit code.
 *
 * @param {string} phoneNumber - E.164 format e.g. "+12125550100" or "+2349084467821"
 * @param {string} [friendlyName] - Human-readable label for this caller ID
 * @returns {{ validationCode: string, callSid: string, phoneNumber: string }}
 */
async function verifyCallerIdStart(
  phoneNumber,
  friendlyName,
  statusCallbackUrl,
) {
  const params = {
    PhoneNumber: phoneNumber,
    FriendlyName: friendlyName || `Agently verified number`,
  };

  // If we have a public callback URL, Twilio will POST the verification result
  // to it when the call ends — includes VerificationStatus=success/failed.
  if (statusCallbackUrl) {
    params.StatusCallback = statusCallbackUrl;
    params.StatusCallbackMethod = "POST";
  }

  const data = await twilioRequest("POST", "/OutgoingCallerIds.json", params);
...
/**
 * Check whether a number has been verified as an OutgoingCallerId.
 * Call this AFTER the user has answered the Twilio verification call.
 *
 * @param {string} phoneNumber - E.164 format
 * @returns {{ verified: boolean, callerIdSid: string|null, phoneNumber: string }}
 */
async function verifyCallerIdCheck(phoneNumber) {
  // GET /OutgoingCallerIds.json?PhoneNumber=+xxx
  ...
  if (ids.length === 0) {
    return { verified: false, callerIdSid: null, phoneNumber };
  }

  return {
    verified: true,
    callerIdSid: ids[0].sid,
    phoneNumber: ids[0].phone_number || phoneNumber,
  };
}

// ─────────────────────────────────────────────────────────────
// Twilio Media Streams + Browser Voice SDK helpers
// ─────────────────────────────────────────────────────────────
...
function createVoiceAccessToken({ identity }) {
  ...
  if (!accountSid || !apiKeySid || !apiKeySecret || !twimlAppSid) {
    throw new Error(
      "Missing TWILIO_ACCOUNT_SID, TWILIO_API_KEY_SID, TWILIO_API_KEY_SECRET, or TWILIO_TWIML_APP_SID.",
    );
  }
  ...
  const payload = {
    ...
    exp: now + 3600,
    grants: {
      identity: String(identity || "agently-user").slice(0, 120),
      voice: {
  ...
  return jwt.sign(payload, apiKeySecret, {
    algorithm: "HS256",
    header: { typ: "JWT", cty: "twilio-fpa;v=1" },
  });
}
...
module.exports = {
  ...
  listOwnedNumbers,
  // ── Caller ID Verification (for user-owned external numbers) ──
  // verifyCallerIdStart: initiates a Twilio verification call to a number.
  //   Returns { validationCode, callSid, phoneNumber } immediately.
  //   Twilio calls the number and reads the validationCode to the recipient.
  verifyCallerIdStart,
  // verifyCallerIdCheck: checks whether Twilio has confirmed a number as verified.
  //   Returns { verified: boolean, callerIdSid: string|null }
  verifyCallerIdCheck,
  // Media Streams / browser voice
  buildMediaStreamTwiml,
  createVoiceAccessToken,
  // Legacy ConversationRelay helpers retained only so older non-voice routes do not crash.
