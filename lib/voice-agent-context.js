...
function scoreText(tokens, text) {
  ...
  return tokens.reduce((score, tok) => {
    if (hay.includes(tok)) return score + (tok.length > 5 ? 3 : 1);
    if (!haystackWords) {
      haystackWords = [...new Set(hay.split(/[^a-z0-9]+/).filter(Boolean))];
    }
    return fuzzyTokenMatches(tok, haystackWords)
      ? score + (tok.length > 5 ? 1 : 1)
      : score;
  }, 0);
}
...
function buildInstructionBlock(context) {
  ...
  return [
    ...
    "- Use only the selected Knowledge Base context. If information is missing, never say you lack information or mention any knowledge base to the caller — help with the closest relevant thing available, or warmly offer a follow-up. Do not use another website or another Knowledge Base as fallback.",
