...
async function loadFaqs(db, tenantId, agentId) {
  const rows = await safeQuery(
    ...
    () =>
      db
        .from("faqs")
        .select("id,question,answer,voice_agent_id,knowledge_base_id")
        .eq("organization_id", tenantId)
...
async function loadLinkedChatbotData(db, tenantId, agentId) {
  const bots = await safeQuery(
    ...
    () =>
      db
        .from("chatbots")
        .select("id,name,header_title,welcome_message,custom_prompt,faqs")
        .eq("organization_id", tenantId)
        .eq("voice_agent_id", agentId)
        .limit(10),
    [],
...
async function loadChunks(db, tenantId, agentId, chatbotIds) {
  const direct = await safeQuery(
    ...
    () =>
      db
        ...
        .select(
          "id,source_url,source_title,content,chunk_index,voice_agent_id,chatbot_id",
        )
        .eq("organization_id", tenantId)
        .eq("voice_agent_id", agentId)
        .limit(120),
  ...
  const orgFallback = await safeQuery(
    "tenant-knowledge",
    () =>
      db
        .from("knowledge_chunks")
  ...
  const seen = new Set();
  return [...(direct || []), ...(linked || []), ...(orgFallback || [])].filter(
    (row) => {
...
async function getKnowledgeContextForCall({
  ...
  const linked = agent
    ? await loadLinkedChatbotData(db, tenantId, agent.id)
  ...
  const chunks = agent
    ? await loadChunks(db, tenantId, agent.id, linked.chatbotIds)
    : [];
  const scoringQuery = [
    query,
    callPurpose,
