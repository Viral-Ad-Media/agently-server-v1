...
function serializeSchedule(row = {}) {
  ...
  const scheduleConfig = jsonObject(
    metadata.scheduleConfig || row.schedule_config,
  );
  return {
    id: row.id,
    ...
    scheduleDates: Array.isArray(metadata.scheduleDates)
      ? metadata.scheduleDates
      : Array.isArray(scheduleConfig.scheduleDates)
        ? scheduleConfig.scheduleDates
        : [],
    scheduleWindows: Array.isArray(metadata.scheduleWindows)
      ? metadata.scheduleWindows
      : Array.isArray(scheduleConfig.scheduleWindows)
        ? scheduleConfig.scheduleWindows
        : Array.isArray(row.windows)
          ? row.windows
          : [],
    windows: Array.isArray(row.windows) ? row.windows : [],
    effectiveLimits: row.effective_limits || metadata.effectiveLimits || null,
...
function safeOutcomeMetadata(row = {}) {
  return jsonObject(row.outcome_metadata || row.metadata || {});
}

function detailedOutcome(row = {}) {
  const meta = safeOutcomeMetadata(row);
  const provider = jsonObject(meta.provider);
  const twilioStatus = String(
    meta.twilioCallStatus || meta.callStatus || meta.twilio_status || "",
  ).toLowerCase();
  const outcome = String(
    meta.outcome || meta.outcomeStatus || meta.displayOutcome || "",
  ).toLowerCase();
  const deferredReason = String(
    meta.deferred_reason || meta.deferredReason || "",
  ).toLowerCase();
  ...
  const answeredBy = String(
    meta.answeredBy || meta.answered_by || meta.machine_detection_result || "",
  ).toLowerCase();
  const duration =
    Number(
      row.call_duration ||
  ...
  if (
    ...
    errorCode === "ONE_TIME_CONCURRENCY_LIMIT_EXCEEDED"
  )
    return "one_time_limit_exceeded";
  if (
    deferredReason === "concurrency_limit" ||
    errorCode === "ORG_CONCURRENCY_LIMIT"
  )
    return "deferred_concurrency_limit";
  if (outcome) return outcome;
  if (twilioStatus === "busy") return "busy";
  if (twilioStatus === "no-answer") return "no_answer";
  if (["failed", "canceled", "cancelled"].includes(twilioStatus))
    return "failed";
  if (
    /machine|fax/.test(answeredBy) ||
    meta.voicemailDetected === true ||
    meta.voicemail_detected === true
  )
    return "voicemail";
  if (twilioStatus === "completed" && duration > 0) return "answered";
  if (twilioStatus === "completed") return "completed";
  if (status === "completed") return duration > 0 ? "answered" : "completed";
  if (status === "initiated") return "initiated";
  if (status === "failed") return "failed";
  return "queued";
}

function displayStatusForRun(row = {}) {
  const outcome = detailedOutcome(row);
  const labels = {
    queued: "Queued",
    deferred_concurrency_limit: "Deferred — waiting for available call slot",
    initiated: "Calling",
    ringing: "Ringing",
    answered: "Answered",
    completed: "Completed",
    ...
    busy: "Busy",
    failed: "Failed",
    one_time_limit_exceeded: "One-time call limit exceeded — add to next batch",
    cancelled: "Cancelled",
    retry_scheduled: "Retry scheduled",
  };
  return labels[outcome] || labels[row.status] || "Queued";
}

function durationSecondsForRun(row = {}) {
  const meta = safeOutcomeMetadata(row);
  const values = [
    row.call_duration,
    meta.callDuration,
...
function retryAtForRun(row = {}) {
  const meta = safeOutcomeMetadata(row);
  if (meta.retryAt) return meta.retryAt;
  if (detailedOutcome(row) === "deferred_concurrency_limit")
    return "next scheduler tick";
  if (row.status === "queued" && row.scheduled_for) return row.scheduled_for;
  return null;
}

function serializeRun(row = {}) {
  const outcomeMetadata = safeOutcomeMetadata(row);
  const outcome = detailedOutcome(row);
  const durationSeconds = durationSecondsForRun(row);
  return {
    id: row.id,
    organizationId: row.organization_id,
    ...
    status: row.status,
    displayStatus: displayStatusForRun(row),
    outcome,
    durationSeconds,
    retryAt: retryAtForRun(row),
