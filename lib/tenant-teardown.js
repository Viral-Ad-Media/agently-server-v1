...
/**
 * Full tenant teardown: release provider resources FIRST, then purge the
 * database — never the other way round.
 *
 * super-admin.js has always imported `teardownTenant` from this module, but
 * this module only ever exported the two halves, so the call resolved to
 * undefined and organization deletion failed with "teardownTenant is not a
 * function" every time it was attempted. This is the orchestration that was
 * missing.
 *
 * Order matters and is the whole point: the database rows are the only record
 * of which numbers exist. If the purge ran first and a release then failed,
 * the number would keep billing forever with nothing left to identify it. So
 * a provider failure aborts before the purge, unless the caller explicitly
 * passes allowPartial.
 */
async function teardownTenant({
  ...
  if (!organizationId) {
    const error = new Error("organizationId is required for teardown.");
    error.status = 400;
    error.code = "ORGANIZATION_REQUIRED";
    throw error;
  }

  const provider = await releaseTenantProviderResources({ organizationId });

  if (!provider.providerFullyReleased && !allowPartial) {
    const error = new Error(
      `Could not release every Twilio resource for this organization: ` +
        `${provider.failedCount} number(s) and ` +
        `${provider.failedSubaccounts.length} subaccount(s) failed. ` +
        `Nothing was deleted, so the records of what still needs releasing are intact. ` +
        `Resolve the failures, or retry with allowPartial to delete anyway.`,
    );
    error.status = 409;
    error.code = "PROVIDER_RELEASE_INCOMPLETE";
    error.details = {
      failed: provider.failed,
      failedSubaccounts: provider.failedSubaccounts,
    };
    throw error;
  }
  ...
  const { data, error: rpcError } = await db.rpc(
    "billing_admin_delete_user_or_org_everything",
    {
      ...
      p_delete_scope: "organization",
      p_confirm: confirm,
    },
  );
  if (rpcError) {
    const error = new Error(
      rpcError.message || "The database purge failed after releasing numbers.",
    );
    error.status = 500;
    error.code = "PURGE_FAILED";
    // Say plainly what already happened upstream, so an operator is not left
    // guessing whether the numbers are still rented.
    error.details = {
      providerAlreadyReleased: provider.released,
      closedSubaccounts: provider.closedSubaccounts,
    };
    throw error;
  }
  ...
  return {
    ...
    adminEmail,
    numbers: {
      released: provider.releasedCount,
      detail: provider.released,
      failed: provider.failed,
    },
    subaccounts: {
      closed: provider.closedSubaccounts,
      failed: provider.failedSubaccounts,
    },
    database: data || null,
    partial: !provider.providerFullyReleased,
  };
}
